...
function avisExpiresOn(sh){const picked=berlinDateKey(dateTimeOf(sh));return picked?addCalendarDays(picked,3):''}
...
async function savePendingCustomerUpload(teamBlob,sessionInfo,session,entry,firstRead){
 for(let i=0;i<MAX_RETRIES;i++){
  ...
  team.state=state;team.revision=Number(team.revision||0)+1;team.updatedAt=now();team.updatedBy='Kunden-Avis PDF-Upload';team.updatedByUserId='customer-avis';team.clientVersion='RC1128';
  try{await writeTeam(teamBlob,team,d.etag);return publicShipment(findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference),session)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(80+i*100);continue}throw e}
 }
...
async function updateCustomerUploadOutcome(teamBlob,sessionInfo,session,entry,file){
 for(let i=0;i<MAX_RETRIES;i++){
  ...
  team.state=state;team.revision=Number(team.revision||0)+1;team.updatedAt=now();team.updatedBy=file?'Kunden-Avis Dokument gespeichert':'Kunden-Avis Dokument blockiert';team.updatedByUserId='customer-avis';team.clientVersion='RC1128';
  try{await writeTeam(teamBlob,team,d.etag);return publicShipment(findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference),session)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(80+i*100);continue}throw e}
 }
 throw error('STATE_CONFLICT','PDF-Prüfergebnis konnte nicht gespeichert werden.',409)
}
async function promoteCleanCustomerPdf(teamBlob,sessionInfo,session,quarantineBlob,uploadId,scan){
 ...
 const file={id:'customer-avis-'+uploadId.slice(0,20),name,category:'Anhang',mimeType:'application/pdf',type:'application/pdf',size:bytes.length,storage:'blob',blobName,sha256:uploadId,customerAvisVisible:true,source:'customer-avis-upload',uploadedAt,uploadedBy:'Kunde via Lieferavis',malwareScan:{provider:'Microsoft Defender for Storage',result:'No threats found',scanTime:text(scan.scanTime)}};
 const shipment=await updateCustomerUploadOutcome(teamBlob,sessionInfo,session,{id:uploadId,name,size:bytes.length,status:'saved',uploadedAt:text(metadata.uploadedat),completedAt:uploadedAt,message:'PDF geprüft und gespeichert.',scanResult:text(scan.result),scanTime:text(scan.scanTime)},file);
 try{if(typeof quarantineBlob.deleteIfExists==='function')await quarantineBlob.deleteIfExists()}catch(_){}
 return{ok:true,status:'saved',upload:{id:uploadId,name,size:bytes.length,status:'saved',scanResult:text(scan.result),scanTime:text(scan.scanTime)},shipment}
}
...
async function customerPdfScanStatus(teamBlob,sessionInfo,session,sh,uploadId){
 ...
 return{ok:true,status:'scanning',upload:{id:uploadId,name:text(queueEntry&&queueEntry.name)||'Kunden-Dokument.pdf',size:Number(queueEntry&&queueEntry.size||0)||0,status:'scanning',provider:'Microsoft Defender for Storage'},shipment:publicShipment(sh,session)}
}
...
module.exports=async function(context,req){
 ...
 try{
  ...
  if(req.method==='POST'&&(action==='issue'||action==='disable')){
   ...
   if(action==='disable'){await access.revokeSubject(req,'avis',actualSubject,'disabled',internal.user.name||internal.user.user||'ExportHUB',payload);if(!draftOnly){const flagStarted=Date.now();await setAvisFlags(blob,actualSubject,actualRef,false,internal.user.name||internal.user.user,d);timing.flagWriteMs=elapsed(flagStarted)}timing.totalMs=elapsed(requestStarted);context.res=json(200,{ok:true,disabled:true,shipmentId:actualSubject,reference:actualRef,timing,version:'RC1128'},timingHeaders(timing));return}
   ...
   timing.totalMs=elapsed(requestStarted);context.res=json(200,{ok:true,issued:true,token:issued.token,shipmentId:actualSubject,reference:actualRef,expiresAt:issued.expiresAt,url:'/customer-avis.html?token='+encodeURIComponent(issued.token)+'&environment='+encodeURIComponent(env),oneTime:false,timing,version:'RC1128'},timingHeaders(timing));return
  }
  ...
  if(req.method==='POST'){
   ...
   if(postAction==='upload-document'){
    ...
    try{
     const shipment=await savePendingCustomerUpload(blob,sessionInfo,session,{id:validated.sha256,name:validated.name,size:validated.size,status:'scanning',uploadedAt:quarantine.uploadedAt,message:'PDF wird auf Schadsoftware geprüft.'},d);
     context.res=json(202,{ok:true,status:'scanning',upload:{id:validated.sha256,name:validated.name,size:validated.size,status:'scanning',provider:'Microsoft Defender for Storage'},shipment});return
   ...
   if(postAction==='document-upload-status'){
    context.res=json(200,await customerPdfScanStatus(blob,sessionInfo,session,sh,lower(payload.uploadId)));return
   }
 ...
 }catch(e){context.log&&context.log.error&&context.log.error('customer-avis RC1128',e&&e.code,e&&e.message);context.res=json(e.status||e.statusCode||500,{ok:false,code:e.code||'SERVER_ERROR',message:e.message||'Kunden-Avis ist vorübergehend nicht verfügbar.'})}
};