...
test('Kundenmail ersetzt bei aktivem Lieferavis die Sendungsdetails',()=>{
  for(const [name,source] of [['Produktion',productionHtml],['TESTVERSION',html]]){
    ...
    assert.match(fn,/Lieferavis-Link bleibt bis einschließlich 14 Kalendertage nach der tatsächlichen Abholung verfügbar|delivery notice link remains available through 14 calendar days after the actual collection/,`${name}: AVIS-Hinweis fehlt`);
