...
test('RC1266: strukturierte Lieferadresse wird für UPS erkannt',()=>{
  const x=load({__EXPORTHUB_GET_STATE__:()=>({currentShipment:{deliveryAddress:{postalCode:'60044',city:'Fabriano',country:'Italia'}}})});
  assert.deepEqual(JSON.parse(JSON.stringify(x.shipmentDestination())),{country:'IT',address:'60044 Fabriano Italia',postal:'60044'});
});

test('RC1266: Kartonanzahl und Gesamtgewicht fallen auf Sendungsdaten zurück',()=>{
  const x=load({__EXPORTHUB_GET_STATE__:()=>({currentShipment:{rows:[{type:'Karton',count:2,weight:20},{type:'Karton',count:1,weight:12}]}})});
