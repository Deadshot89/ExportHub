...
test('RC1160: AES-256-GCM speichert Portal-Secrets nicht im Klartext',()=>{
  const encrypted=store.encryptSecret('SECRET-USER-RC1160',key);
  const serialized=JSON.stringify(encrypted);
  assert.doesNotMatch(serialized,/SECRET-USER-RC1160/);
  assert.equal(encrypted.alg,'aes-256-gcm');
  assert.equal(store.decryptSecret(encrypted,key),'SECRET-USER-RC1160');
});
test('RC1160: manipuliertes GCM-Tag wird fail-closed abgewiesen',()=>{
  const encrypted=store.encryptSecret('SECRET-PASS-RC1160',key);
...
test('RC1160: Metadaten enthalten niemals Ciphertext oder Klartext-Zugangsdaten',()=>{
...
test('RC1160: nur HTTPS-Portale sind zulässig und URL-Credentials werden entfernt',()=>{
...
test('RC1160: TESTSERVICE nutzt eigenes Credential-Blob',()=>{
