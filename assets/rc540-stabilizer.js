(function(){
'use strict';
if(window.__EXPORTHUB_RC540_STABILIZER__)return;
window.__EXPORTHUB_RC540_STABILIZER__=true;
...
function signature(v,r){
 if(!r)return'';
 if(v==='shipment')return [v,!!r.querySelector('#rc532OperationsStack'),!!r.querySelector('#rc524MailArea'),!!r.querySelector('#rc524StowPlan'),!!r.querySelector('#rc532ProcessActions')].join('|');
...
function setVersionEverywhere(){
 try{document.title='ExportHUB RC540'}catch(_){ }
 document.documentElement.setAttribute('data-exporthub-version','RC540');
 document.querySelectorAll('[data-exporthub-version-label],[id*=version i],[class*=version i]').forEach(function(n){
  ...
  if(/(?:ExportHUB\s+Private\s+|Private\s+)?RC\d+/i.test(t))n.textContent=t.replace(/ExportHUB\s+Private\s+RC\d+/gi,'ExportHUB RC540').replace(/Private\s+RC\d+/gi,'RC540').replace(/RC\d+/gi,'RC540');
 });
}
function needsRepair(v,r){
 if(!r)return false;
 if(v==='shipment')return !r.querySelector('#rc532OperationsStack')||!r.querySelector('#rc524MailArea')||!r.querySelector('#rc524StowPlan')||!r.querySelector('#rc532ProcessActions');
 if(v==='abd')return !r.classList.contains('rc540-abd-view')||!r.querySelector('.rc540-abd-page');
 ...
 if(v==='update')return !r.classList.contains('rc524-update-view')||!/RC540/.test(r.textContent||'');
...
function repair(reason){
 ...
 repairing=true;
 try{
  var core=window.ExportHUBRC540||window.ExportHUBRC538||window.ExportHUBRC532;
  ...
  window.__EXPORTHUB_RC538_UI_STATUS__={ok:true,view:v,reason:reason||'repair',version:'RC540',checkedAt:new Date().toISOString(),areas:{shipmentOrder:'Kunde/Grunddaten > Collis > Stauplan > Mail > Reports > Gespeicherte Sendungen',abd:'eigener Bereich'}};
 }catch(e){console.error('RC540 Oberflächenreparatur',e);window.__EXPORTHUB_RC538_UI_STATUS__={ok:false,error:String(e&&e.message||e),view:v}}
...
function installViewWrapper(){
 ...
 function wrapped(v){
  ...
  var result=false;
  try{if(typeof old==='function')result=old.apply(this,arguments);else if(typeof window.render==='function')result=window.render()}catch(e){console.error('RC540 Ansichtswechsel',e)}
