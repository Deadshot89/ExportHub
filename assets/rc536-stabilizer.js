(function(){
'use strict';
if(window.__EXPORTHUB_RC536_STABILIZER__)return;
window.__EXPORTHUB_RC536_STABILIZER__=true;
...
function setVersionEverywhere(){
 try{document.title='ExportHUB RC536'}catch(_){ }
 document.documentElement.setAttribute('data-exporthub-version','RC536');
 document.querySelectorAll('[data-exporthub-version-label],[id*=version i],[class*=version i]').forEach(function(n){
  ...
  if(/(?:ExportHUB\s+Private\s+|Private\s+)?RC\d+/i.test(t))n.textContent=t.replace(/ExportHUB\s+Private\s+RC\d+/gi,'ExportHUB RC536').replace(/Private\s+RC\d+/gi,'RC536').replace(/RC\d+/gi,'RC536');
 });
}
function needsRepair(v,r){
 ...
 if(v==='update')return !r.classList.contains('rc524-update-view')||!/RC536/.test(r.textContent||'');
...
function repair(reason){
 ...
 repairing=true;
 try{
  var core=window.ExportHUBRC536||window.ExportHUBRC532;
  ...
  window.__EXPORTHUB_RC536_UI_STATUS__={ok:true,view:v,reason:reason||'repair',version:'RC536',checkedAt:new Date().toISOString(),areas:{shipmentOrder:'Kunde/Grunddaten > Collis > Stauplan > Mail > Reports > Gespeicherte Sendungen'}};
 }catch(e){console.error('RC536 Oberflächenreparatur',e);window.__EXPORTHUB_RC536_UI_STATUS__={ok:false,error:String(e&&e.message||e),view:v}}
...
function installViewWrapper(){
 ...
 if(old&&old.__rc536Wrapped)return;
 function wrapped(v){
  var st=state();if(st)st.view=String(v||'dashboard');
  var result=false;
  try{if(typeof old==='function')result=old.apply(this,arguments);else if(typeof window.render==='function')result=window.render()}catch(e){console.error('RC536 Ansichtswechsel',e)}
  schedule('Ansichtswechsel');return result;
 }
 wrapped.__rc536Wrapped=true;wrapped.__rc536Original=old;window.setView=wrapped;try{setView=wrapped}catch(_){ }
}
