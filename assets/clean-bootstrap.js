(function(){
'use strict';
const BUILD=window.EXPORTHUB_BUILD||{version:'RC535',cache:'535',loginReturn:'/?v=535'};
const VERSION=String(BUILD.version||'RC535');
...
function rc524StyleHealth(){
 ...
 let link=by('rc535MainStyles');
 if(!link){link=document.createElement('link');link.id='rc535MainStyles';link.rel='stylesheet';document.head.appendChild(link)}
 link.href='assets/exporthub-ui-rc535.css?v='+CACHE+'&retry='+Date.now();
 return fetch('assets/exporthub-ui-rc535.css?v='+CACHE,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('CSS HTTP '+r.status);return r.text()}).then(function(css){
  ...
  return true
 }).catch(function(e){console.error('RC535 Design konnte nicht nachgeladen werden',e);return false})
}
...
function applyRemoteDocument(doc,reason){if(!doc||!isObject(doc.state)||(runtime.saving&&reason!=='concurrent-merge'))return false;canonicalizeCustomers(doc.state);runtime.applyingRemote=true;try{const state=window.__EXPORTHUB_GET_STATE__?window.__EXPORTHUB_GET_STATE__():runtime.state;if(isObject(state))mutateObject(state,doc.state,true);runtime.state=state||clone(doc.state);const targetUsers=window.__EXPORTHUB_GET_USERS__?window.__EXPORTHUB_GET_USERS__():runtime.users;if(Array.isArray(targetUsers)){targetUsers.splice.apply(targetUsers,[0,targetUsers.length].concat((Array.isArray(doc.users)?doc.users:[]).map(clone)));runtime.users=targetUsers}else runtime.users=clone(doc.users||[]);runtime.revision=Number(doc.revision||runtime.revision);runtime.lastSnapshot=clone(doc.state);runtime.lastUsers=clone(doc.users||runtime.users);runtime.dirty=false;runtime.remoteApplyCount++;window.__CLEAN_BOOT_USERS__=runtime.users;refreshCurrentUser();if((window.ExportHUBRC535||window.ExportHUBRC532)&&typeof (window.ExportHUBRC535||window.ExportHUBRC532).restoreUsers==='function')(window.ExportHUBRC535||window.ExportHUBRC532).restoreUsers();if(typeof window.render==='function')window.render();setVersion();window.dispatchEvent(new CustomEvent('exporthub:sync',{detail:{revision:runtime.revision,reason:reason||'poll'}}));return true}finally{runtime.applyingRemote=false}}
...
async function loadLegacy(){
 ...
 try{if((window.ExportHUBRC535||window.ExportHUBRC532)&&typeof (window.ExportHUBRC535||window.ExportHUBRC532).install==='function')(window.ExportHUBRC535||window.ExportHUBRC532).install()}catch(e){console.error('RC535 Konsolidierung konnte nicht aktiviert werden',e);throw e}
