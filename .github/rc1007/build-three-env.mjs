...
const OUT=path.join(ROOT,'dist-rc1007');
const VERSION='RC1007';
const CACHE='1007';
...
function setVersion(html){
  let out=html.replace(/var BUILD=Object\.freeze\(\{version:'RC\d+',cache:'\d+',loginReturn:'([^']*)'\}\);/,(_m,ret)=>{
    const next=String(ret||'').replace(/v=\d+/,'v=1007');
...
function envTag(env){return `<script id="exporthub-rc1007-env-config">window.__EXPORTHUB_FORCED_ENVIRONMENT__=${JSON.stringify(env)};<\/script>\n<script id="exporthub-rc1007-env-hub" defer src="${HUB_SRC}"><\/script>`;}
function demoTag(){return `<script id="exporthub-rc1007-demo-bootstrap" src="${DEMO_SRC}"><\/script>`;}
...
function demoAsset(){
  return read('assets/exporthub-demo-bootstrap.js')
    .replace(/revision:\d+/g,'revision:1007')
    .replace(/clientVersion:'RC\d+-demo'/g,"clientVersion:'RC1007-demo'")
    .replace(/version:'RC\d+'/g,"version:'RC1007'")
    .replace(/eh\d+-demo-banner/g,'eh1007-demo-banner');
}

const production=marker(injectBeforeHeadClose(setVersion(read('index.html')),envTag('production'),'exporthub-rc1007-env-config'),'production-candidate');
const testservice=marker(injectBeforeHeadClose(setVersion(read('TESTVERSION.html')),envTag('testservice'),'exporthub-rc1007-env-config'),'testservice');
const demo=marker(injectBeforeHeadClose(injectAfterHeadOpen(setVersion(read('TESTVERSION.html')),demoTag(),'exporthub-rc1007-demo-bootstrap'),envTag('demo'),'exporthub-rc1007-env-config'),'demo');
