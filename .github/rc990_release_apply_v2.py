...
'''
new='''build_re=re.compile(r"var\\s+BUILD\\s*=\\s*Object\\.freeze\\(\\{version:\\s*'RC980'\\s*,\\s*cache:\\s*'980'\\s*,\\s*loginReturn:\\s*'/TESTVERSION\\.html\\?v=\\d+'\\s*\\}\\);",re.S)
matches=list(build_re.finditer(s))
assert len(matches)==1, f'expected one structural RC980 BUILD with numeric loginReturn, got {len(matches)}'
