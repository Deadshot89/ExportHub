...
function publicShipment(sh,session){
 ...
 if(actual)return{ok:true,closed:true,status:'Abgeholt',reference:sref(sh),actualPickupAt:actual,pickup:{carrier:pickupCarrierOf(sh),licensePlate:pickupPlateOf(sh),driverName:pickupDriverOf(sh)},documents:docs,pod:{available:podAvailable(sh),status:text(sh.podStatus)||(podAvailable(sh)?'POD vorhanden':'Noch nicht vorhanden'),documents:podDocs},avis:{enabled:true,closed:true,singleUse:false,session:true},lastUpdatedAt:text(sh.updatedAt||sh._syncUpdatedAt||sh.modifiedAt)};
...
module.exports=async function(context,req){
 ...
 try{
  ...
  if(req.method==='POST'&&(action==='issue'||action==='disable')){
   ...
   if(action==='disable'){await access.revokeSubject(req,'avis',actualSubject,'disabled',internal.user.name||internal.user.user||'ExportHUB',payload);if(!draftOnly){const flagStarted=Date.now();await setAvisFlags(blob,actualSubject,actualRef,false,internal.user.name||internal.user.user,d);timing.flagWriteMs=elapsed(flagStarted)}timing.totalMs=elapsed(requestStarted);context.res=json(200,{ok:true,disabled:true,shipmentId:actualSubject,reference:actualRef,timing,version:'RC1036'},timingHeaders(timing));return}
  ...
  if(req.method==='POST'){const postAction=lower(payload.action||'appointment');if(postAction!=='appointment')throw error('UNKNOWN_ACTION','Unbekannte Avis-Aktion.',400);context.res=json(200,await saveAppointment(blob,sessionInfo,session,payload,d));return}
  context.res=json(405,{ok:false,code:'METHOD_NOT_ALLOWED'},{Allow:'GET, POST, OPTIONS'});
 }catch(e){context.log&&context.log.error&&context.log.error('customer-avis RC1036',e&&e.code,e&&e.message);context.res=json(e.status||e.statusCode||500,{ok:false,code:e.code||'SERVER_ERROR',message:e.message||'Kunden-Avis ist vorübergehend nicht verfügbar.'})}
};