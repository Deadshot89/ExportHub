...
function buildAvisBody(body,url,reference,target,lang){
 ...
 if(lang==='en'){
  ...
  followup=carrier?'Please submit the pickup information through the collection notice. A separate confirmation of the shipment details by email is not required.':'Please use the collection notice for the pickup information. The shipment details are therefore not repeated in this email.';
 }else{
  ...
  followup=carrier?'Bitte übermitteln Sie die Abholdaten über den Lieferavis. Eine zusätzliche Bestätigung der Sendungsdetails per E-Mail ist nicht erforderlich.':'Bitte verwenden Sie für die Abholdaten den Lieferavis. Die Sendungsdetails werden deshalb in dieser E-Mail nicht zusätzlich wiederholt.';
 }
