...
test('RC1027: Standarddeploy führt Release-Vorbereitung vor dem gemeinsamen Drei-Umgebungen-Build aus',()=>{
  ...
  const buildAt=WORKFLOW.indexOf('node .github/rc1045/build-three-env.mjs');
  assert.ok(patchAt>=0&&buildAt>patchAt,'RC1027-Injektion muss vor dem aktuellen gemeinsamen Build stattfinden.');
  assert.match(fs.readFileSync(path.join(ROOT,'.github/rc1045/build-three-env.mjs'),'utf8'),/\.github\/rc1044\/build-three-env\.mjs/,'RC1045 muss den geprüften RC1044-Vorgänger verwenden.');
