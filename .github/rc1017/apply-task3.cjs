...
const helpers=`function rc1017SubShipmentLocked(sh){
 sh=sh||{};
...
if(!html.includes('function rc1017FitRows(')){
...
if(!html.includes("renderRc1017SubShipments(card,shipment());\n var vs=card.querySelector('#rc713VehicleType')")){
  html=replaceOnce(
    html,
    " var vs=card.querySelector('#rc713VehicleType');",
    " renderRc1017SubShipments(card,shipment());\n var vs=card.querySelector('#rc713VehicleType');",
    'RC1017 Zusammenfassung im Stauplan'
  );
}
...
if(!build.includes("'assets/rc1017-multi-truck.js'")){
  build=replaceOnce(
    ...
    'RC1017 Assetkopie im Drei-Umgebungen-Build'
  );
}
fs.writeFileSync(buildPath,build);

console.log('RC1017 Task 3 angewendet: Stauplanadapter, Persistenz, Lock und Mehr-LKW-Zusammenfassung.');
