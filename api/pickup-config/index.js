...
async function adminAllowed(req){
  ...
  return policy.isAdmin(u)||policy.isFunctionAdmin(u,'qr');
}
module.exports=async function(context,req){
  try{
    ...
    if(!(await adminAllowed(req))){context.res=store.json(403,{ok:false,code:'ADMIN_REQUIRED',message:'Nur globale Administratoren oder QR-Funktions-Admins dürfen die zentrale QR-PIN ändern.'});return}
