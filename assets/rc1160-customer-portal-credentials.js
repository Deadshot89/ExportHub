...
function readinessBadge(ready,adminView){
 if(ready&&ready.configured===true)return'<span class="rc1160-badge ok">'+th('portal.encryptionActive')+'</span>';
 return'<span class="rc1160-badge off">'+th(adminView?'portal.serverKeyMissing':'portal.encryptionNotReady')+'</span>'
}
function readinessHint(ready,adminView){
 if(ready&&ready.configured===true)return'';
 return'<div class="rc1160-muted" data-rc1162-key-warning>'+th(adminView?'portal.adminKeyWarning':'portal.userKeyWarning')+'</div>'
}
function portalRows(customer,portals,canManage,configured){
 if(!portals.length)return'<div class="rc1160-muted">'+th('portal.noneConfigured')+'</div>';
 return'<div class="rc1160-list">'+portals.map(function(p){
  return'<div class="rc1160-row" data-portal-id="'+esc(p.id)+'"><div class="rc1160-row-top"><div><b>'+esc(p.name)+'</b><div class="rc1160-muted">'+esc(p.url)+'</div></div><span class="rc1160-badge '+(p.active!==false?'ok':'off')+'">'+th(p.active!==false?'common.active':'common.inactive')+'</span></div>'+
   '<div><span class="rc1160-badge">'+th(p.hasUsername?'portal.usernameSet':'portal.usernameMissing')+'</span> <span class="rc1160-badge">'+th(p.hasPassword?'portal.passwordSet':'portal.passwordMissing')+'</span></div>'+
   '<div class="rc1160-actions"><button type="button" class="btn" data-rc1160-open="'+esc(p.id)+'">'+th('portal.open')+'</button><button type="button" class="btn primary" data-rc1160-reveal="'+esc(p.id)+'" '+(configured?'':'disabled title="'+th('portal.encryptionNotReady')+'"')+'>'+th('portal.showCredentials')+'</button>'+
   (canManage?'<button type="button" class="btn" data-rc1160-edit="'+esc(p.id)+'">'+th('portal.edit')+'</button><button type="button" class="btn" data-rc1160-delete="'+esc(p.id)+'">'+th('portal.delete')+'</button>':'')+'</div></div>'
 }).join('')+'</div>'
}
...
async async function reveal(customer,p){
 ...
 box.querySelector('[data-rc1160-confirm]').addEventListener('click',async function(){
  var password=input.value;input.value='';this.disabled=true;status(out,tr('portal.checkingAccess'));
  try{
   var data=await portalApi('reveal',{customerId:customerId(customer),portalId:p.id,password:password});password='';
   box.querySelector('.rc1160-dialog-card').innerHTML='<h3>'+esc(p.name)+'</h3><p class="rc1160-muted">'+th('portal.autoHide60')+'</p><b>'+th('portal.username')+'</b><div class="rc1160-secret" data-rc1160-secret="username">'+esc(data.username||'')+'</div><button class="btn" data-copy="username">'+th('portal.copyUsername')+'</button><br><br><b>'+th('portal.password')+'</b><div class="rc1160-secret" data-rc1160-secret="password">'+esc(data.password||'')+'</div><button class="btn" data-copy="password">'+th('portal.copyPassword')+'</button><div class="rc1160-actions" style="margin-top:16px"><button class="btn" data-rc1160-close>'+th('common.close')+'</button></div>';
  ...
  }catch(e){password='';status(out,e.message||tr('portal.showFailed'),'bad');this.disabled=false}
 })
}
function portalForm(customer,existing,onDone){
 existing=existing||null;var box=modal('<h3>'+th(existing?'portal.editTitle':'portal.addTitle')+'</h3>'+
 '<label>'+th('portal.portalName')+'<input data-f="name" value="'+esc(existing&&existing.name)+'"></label><label>'+th('portal.httpsAddress')+'<input data-f="url" type="url" value="'+esc(existing&&existing.url)+'"></label>'+
 '<label>'+th('portal.username')+(existing?'<span class="rc1160-muted">'+th('portal.unchangedIfEmpty')+'</span>':'')+'<input data-f="username" autocomplete="off"></label>'+
 '<label>'+th('portal.password')+(existing?'<span class="rc1160-muted">'+th('portal.unchangedIfEmpty')+'</span>':'')+'<input data-f="password" type="password" autocomplete="new-password"></label>'+
 '<label>'+th('portal.internalNote')+'<textarea data-f="note">'+esc(existing&&existing.note)+'</textarea></label><label><input data-f="active" type="checkbox" '+(!existing||existing.active!==false?'checked':'')+'> '+th('common.active')+'</label>'+
 '<div class="rc1160-status" data-rc1160-status></div><div class="rc1160-actions"><button class="btn" data-rc1160-close>'+th('common.cancel')+'</button><button class="btn primary" data-save>'+th('common.save')+'</button></div>');
 box.querySelector('[data-save]').addEventListener('click',async function(){
  ...
  var out=box.querySelector('[data-rc1160-status]');this.disabled=true;status(out,tr('portal.saving'));
  try{
  ...
  }catch(e){status(out,e.message||tr('portal.saveFailed'),'bad');this.disabled=false}
 })
}
async async function deletePortal(customer,p,onDone){
 if(!w.confirm(tr('portal.confirmDelete',{name:p.name})))return;
 try{await portalApi('delete',{customerId:customerId(customer),portalId:p.id});if(onDone)onDone()}
 catch(e){w.alert(e.message||tr('portal.deleteFailed'))}
}
...
async async function renderCustomerFolder(customer,r){
 var root=d.getElementById('content');if(!root)return;
 var old=d.getElementById('rc1160CustomerFolderPortal');if(old)old.remove();
 var box=d.createElement('section');box.id='rc1160CustomerFolderPortal';box.className='rc1160-box';box.innerHTML='<div class="rc1160-head"><div><h3>'+th('portal.title')+'</h3><div class="rc1160-muted">'+th('portal.storageHelp')+'</div></div><span data-rc1162-readiness><span class="rc1160-badge">'+th('portal.checking')+'</span></span>'+(r.manage?'<button class="btn primary" data-add>'+th('portal.add')+'</button>':'')+'</div><div data-ready-hint></div><div data-body class="rc1160-muted">'+th('portal.loading')+'</div>';root.appendChild(box);
 var seq=++loadSeq;
 try{var results=await Promise.all([portalApi('list',{customerId:customerId(customer)}),portalApi('status',{})]),data=results[0],ready=results[1];if(seq!==loadSeq||customerId(customer)!==customerId(selectedCustomer()))return;var portals=arr(data.portals),body=box.querySelector('[data-body]'),badge=box.querySelector('[data-rc1162-readiness]'),hint=box.querySelector('[data-ready-hint]');if(badge)badge.innerHTML=readinessBadge(ready,r.manage);if(hint)hint.innerHTML=readinessHint(ready,r.manage);body.className='';body.innerHTML=portalRows(customer,portals,r.manage,ready.configured===true);wirePortalBox(box,customer,portals,r.manage,function(){schedule(true)});var add=box.querySelector('[data-add]');if(add){add.disabled=ready.configured!==true;if(add.disabled)add.title=tr('portal.encryptionNotReady');add.addEventListener('click',function(){portalForm(customer,null,function(){schedule(true)})})}}
 catch(e){var body=box.querySelector('[data-body]');if(body)body.textContent=e.message||tr('portal.loadFailed')}
}
async async function renderShipment(customer,r){
 ...
 var box=d.createElement('section');box.id='rc1160ShipmentPortal';box.className='rc1160-box';box.innerHTML='<div class="rc1160-head"><div><h3>'+th('portal.title')+'</h3><div class="rc1160-muted">'+th('portal.shipmentHelp')+'</div></div><span data-rc1162-readiness><span class="rc1160-badge">'+th('portal.checking')+'</span></span></div><div data-ready-hint></div><div data-body class="rc1160-muted">'+th('portal.loading')+'</div>';host.appendChild(box);
 var seq=++loadSeq;
...
async async function renderRights(){
 ...
 var box=d.createElement('section');box.id='rc1160PortalRights';box.className='rc1160-box';box.innerHTML='<div class="rc1160-head"><div><h3>'+th('portal.rightsTitle')+'</h3><div class="rc1160-muted">'+th('portal.rightsHelp')+'</div></div><span data-rc1162-readiness><span class="rc1160-badge">'+th('portal.checking')+'</span></span></div><div data-ready-hint></div><div data-body class="rc1160-muted">'+th('portal.loadingUsers')+'</div>';root.appendChild(box);
 var seq=++rightsSeq;
 try{
  ...
  body.innerHTML='<div class="rc1160-rights-grid"><select data-user>'+users.map(function(u){return'<option value="'+esc(u.id)+'">'+esc(u.name||u.user)+'</option>'}).join('')+'</select><label><input type="checkbox" data-use> '+th('portal.use')+'</label><label><input type="checkbox" data-manage> '+th('portal.manage')+'</label><button class="btn primary" data-save>'+th('common.save')+'</button></div><div class="rc1160-status" data-rc1160-status></div>';
  ...
  function sync(){var u=chosen()||{},r=u.rights&&u.rights.customerPortal||{},admin=globalAdmin(u);use.checked=admin||r.use===true||r.manage===true;manage.checked=admin||r.manage===true;use.disabled=admin;manage.disabled=admin;save.disabled=admin;status(out,admin?tr('portal.adminHasBoth'):'')}
