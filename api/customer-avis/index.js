...
const AVIS_SESSION_MS=30*24*60*60*1000;
...
function publicShipment(sh,session){
 ...
 if(actual)return{ok:true,closed:true,status:'Abgeholt',reference:sref(sh),customerName:text(sh.customerName||(sh.customer&&sh.customer.name)),totals:{count:totals.count},actualPickupAt:actual,pickup:{carrier:pickupCarrierOf(sh),licensePlate:pickupPlateOf(sh),driverName:pickupDriverOf(sh)},documents:docs,pod:{available:podAvailable(sh),status:text(sh.podStatus)||(podAvailable(sh)?'POD vorhanden':'Noch nicht vorhanden'),documents:podDocs},avis:{enabled:true,closed:true,singleUse:true,session:true,autoExpiresOn,postPickupDays:3},lastUpdatedAt:text(sh.updatedAt||sh._syncUpdatedAt||sh.modifiedAt)};
...
module.exports=async function(context,req){
 ...
 try{
  ...
  if(req.method==='POST'&&(action==='issue'||action==='disable')){
   ...
   if(action==='disable'){await access.revokeSubject(req,'avis',actualSubject,'disabled',internal.user.name||internal.user.user||'ExportHUB',payload);if(!draftOnly){const flagStarted=Date.now();await setAvisFlags(blob,actualSubject,actualRef,false,internal.user.name||internal.user.user,d);timing.flagWriteMs=elapsed(flagStarted)}timing.totalMs=elapsed(requestStarted);context.res=json(200,{ok:true,disabled:true,shipmentId:actualSubject,reference:actualRef,timing,version:'RC1117'},timingHeaders(timing));return}
