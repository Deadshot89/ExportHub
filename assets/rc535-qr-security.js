(function(){
'use strict';
if(window.__EXPORTHUB_RC535_QR_SECURITY__)return;window.__EXPORTHUB_RC535_QR_SECURITY__=true;
...
function migrate(){
  ...
  if(changed)save('RC535 QR-Zugangsdaten abgeglichen');return changed
}
...
function save(reason){try{if(window.ExportHUBClean&&window.ExportHUBClean.queueSave)window.ExportHUBClean.queueSave(reason||'RC535 QR');else if(typeof window.save==='function')window.save()}catch(e){console.error(e)}}
...
function register(sh,forceRotate){
  ...
  inflight[token]=postRegistration(!!forceRotate,expectedVersion).catch(recoverConflict).then(function(r){
    ...
    save('RC535 QR servergebunden aktiviert');patch();return true
  }).catch(function(e){sh.pickupQrActivating=false;sh.pickupQrError=e.status===401?'Microsoft-Anmeldung erforderlich':Q(e.message||e);save('RC535 QR Aktivierungsfehler');patch();throw e}).finally(function(){delete inflight[token]});
  return inflight[token]
}
function qrHtml(sh){if(used(sh))return '<div class="rc535-qr-message ok">QR bereits verwendet</div>';if(!sh.pickupQrRegistered)return '<div class="rc535-qr-message '+(sh.pickupQrError?'bad':'')+'">'+E(sh.pickupQrActivating?'QR wird aktiviert …':sh.pickupQrError||'QR noch nicht aktiviert')+'</div>';if(!window.ExportHUBQRCode||typeof window.ExportHUBQRCode.svg!=='function')return '<div class="rc535-qr-message bad">Lokaler QR-Generator fehlt.</div>';return window.ExportHUBQRCode.svg(pickupUrl(sh),{className:'rc535-local-qr',label:'QR-Abholscan'})}
function patchDoc(doc){var sh=ensure(current(),false);if(!sh||!Q(sh.ref)||!doc)return;var p1=doc.getElementById('rc420Load1')||doc.querySelector('.rc420-load[data-rc420-load="1"]')||doc.querySelector('.rc390-load');if(p1){var ref=p1.querySelector('.rc420-load-ref,.rc390-load-ref,.rc355-ref')||p1,qr=p1.querySelector('.rc420-qr,.rc390-qr,.rc355-qr,.rc344-qr-only'),sig=[key(sh),Q(sh.pickupToken),!!sh.pickupQrRegistered,!!sh.pickupQrActivating,Q(sh.pickupQrError),used(sh)].join('|');if(!qr){qr=doc.createElement('div');qr.className='rc420-qr rc535-qr-slot'}if(qr.parentNode!==ref)ref.appendChild(qr);qr.classList.remove('empty');qr.setAttribute('data-rc535-secure','1');if(qr.getAttribute('data-rc535-state')!==sig){qr.setAttribute('data-rc535-state',sig);qr.innerHTML=qrHtml(sh)}if(!used(sh)&&!sh.pickupQrRegistered&&!sh.pickupQrActivating&&!Q(sh.pickupQrError))nativeTimeout(function(){register(sh,false).catch(function(){})},0)}var p2=doc.getElementById('rc420Load2')||doc.querySelector('.rc420-load[data-rc420-load="2"]')||doc.querySelectorAll('.rc390-load')[1];if(p2)Array.from(p2.querySelectorAll('.rc420-qr,.rc390-qr,.rc355-qr,.rc344-qr-only,[data-rc535-secure]')).forEach(function(q){q.remove()})}
...
function panelHtml(sh){var isUsed=used(sh),active=!!sh.pickupQrRegistered&&!isUsed,status=isUsed?'bereits verwendet':sh.pickupQrActivating?'Aktivierung läuft':active?'aktiv':sh.pickupQrError?'Fehler':'noch nicht aktiviert',cls=isUsed||active?'green':sh.pickupQrError?'red':'orange',pin=isUsed?'—':Q(sh.pickupPin)||'—',sig=E(panelSig(sh));return '<section id="rc535QrSecurityPanel" data-rc535-state="'+sig+'" class="rc535-qr-security no-print"><div><span class="rc535-kicker">Sicherer Abholscan</span><h3>QR-Code &amp; interne PIN</h3><p>Die PIN ist nur intern sichtbar. Sie steht nicht auf Ladeliste, PDF oder im QR-Link.</p></div><div class="rc535-qr-security-grid"><div><span>Status</span><strong class="'+cls+'">'+E(status)+'</strong></div><div><span>Interne PIN</span><strong class="rc535-pin">'+E(pin)+'</strong></div><div><span>Einmalnutzung</span><strong>'+(isUsed?'verbraucht':'aktiv')+'</strong></div></div><div class="toolbar"><button type="button" class="btn" onclick="return rc535ActivateQr()" '+(isUsed||sh.pickupQrActivating?'disabled':'')+'>'+(active?'QR neu registrieren':'QR aktivieren')+'</button><button type="button" class="ghost" onclick="return rc535CopyPin()" '+(isUsed?'disabled':'')+'>PIN kopieren</button><button type="button" class="ghost" onclick="return rc535CheckQrStatus()">Status prüfen</button></div>'+(sh.pickupQrError?'<div class="badbox">'+E(sh.pickupQrError)+'</div>':'')+'</section>'}
function insertPanel(){var sh=ensure(current(),false),old=document.getElementById('rc535QrSecurityPanel');if(!sh||!Q(sh.ref)){if(old)old.remove();return}var sig=panelSig(sh);if(old&&old.getAttribute('data-rc535-state')===sig)return;var html=panelHtml(sh);if(old){old.outerHTML=html;return}var host=document.getElementById('rc532ProcessActions')||document.getElementById('rc535ProcessActions');if(host)host.insertAdjacentHTML('afterend',html);else if(Q(S().view)==='cmr'){var r=document.getElementById('content'),bundle=r&&r.querySelector('.rc390-docview,.rc390-bundle,#rc420DocumentPage');if(bundle)bundle.insertAdjacentHTML('beforebegin',html)}}
function hookRender(){var fn=window.render;if(typeof fn!=='function'||fn.__rc535Qr)return;baseRender=fn;var w=function(){var out=baseRender.apply(this,arguments);schedule(0);return out};w.__rc535Qr=true;window.render=w;try{render=w}catch(e){}}
function schedule(ms){nativeClear(patchTimer);patchTimer=nativeTimeout(patch,Number(ms)||40)}
function patch(){try{hookRender();migrate();patchDoc(document);var f=document.getElementById('rc420PrintFrame');if(f&&f.contentDocument)patchDoc(f.contentDocument);insertPanel()}catch(e){window.__rc535QrPatchError=Q(e&&e.message||e)}}
function syncStatus(sh){sh=ensure(sh||current(),false);if(!/^[a-f0-9]{48}$/i.test(Q(sh.pickupToken)))return Promise.resolve(false);return api('/pickup-status?token='+encodeURIComponent(Q(sh.pickupToken)),{method:'GET',headers:{}}).then(function(st){sh.pickupCredentialVersion=Number(st.credentialVersion||credentialVersion(sh)||0);sh.pickupPinRevision=Number(st.pinRevision||sh.pickupPinRevision||0);sh.pickupPinLocked=st.pinLocked!==false&&sh.pickupCredentialVersion>0;if(st.confirmedAt){var iso=new Date(st.confirmedAt).toISOString(),day=iso.slice(0,10);sh.actualPickupDate=day;sh.pickedUpAtDate=day;sh.actualPickupTime=new Date(iso).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit',second:'2-digit'});sh.pickedUpAt=iso;sh.pickupStatus='abgeholt';sh.status='erledigt';sh.done=true;sh.completedAt=iso;sh.pickupQrUsed=true;sh.pickupQrUsedAt=iso;sh.podScanConfirmed=true;var remote=A(st.podFiles).map(function(f){return {id:'QR-'+Q(f.id),remoteId:Q(f.id),name:Q(f.name)||'POD.pdf',filename:Q(f.name)||'POD.pdf',url:Q(f.url),type:Q(f.type)||'application/pdf',mimeType:Q(f.type)||'application/pdf',size:Number(f.size||0),uploadedAt:Q(f.uploadedAt),added:Q(f.uploadedAt),remote:true,source:'QR',kind:Q(f.kind)}});if(remote.length){var map={};A(sh.podFiles).concat(remote).forEach(function(f){map[Q(f.remoteId)||Q(f.url)||Q(f.id)||Q(f.name)]=f});sh.podFiles=Object.keys(map).map(function(k){return map[k]});sh.podStatus='POD vorhanden';sh.podCount=sh.podFiles.length}var id=Q(sh.id),ref=Q(sh.ref).toUpperCase();A(S().tasks).forEach(function(t){if(Q(t.area).toLowerCase()==='abholtag'&&((id&&Q(t.linkedShipmentId)===id)||(ref&&Q(t.linkedShipmentRef).toUpperCase()===ref))){t.status='erledigt';t.done=true;t.completedAt=iso;t.doneAt=iso}});save('RC535 QR-Status synchronisiert')}patch();try{if(typeof window.render==='function')window.render()}catch(e){}return !!st.confirmedAt})}
window.rc535Patch=patch;window.rc535ActivateQr=function(){var sh=ensure(current(),false);register(sh,false).catch(function(e){alert('QR-Aktivierung fehlgeschlagen: '+Q(e.message||e))});return false};
window.rc535CopyPin=function(){var sh=ensure(current(),false),pin=Q(sh.pickupPin);if(!pin)return false;try{navigator.clipboard.writeText(pin).then(function(){alert('Interne QR-PIN kopiert.')},function(){prompt('Interne QR-PIN:',pin)})}catch(e){prompt('Interne QR-PIN:',pin)}return false};
window.rc535CheckQrStatus=function(){syncStatus(current()).catch(function(e){alert('QR-Status konnte nicht geprüft werden: '+Q(e.message||e))});return false};
window.rc535SyncPickup=function(){return A(S().shipments).reduce(function(p,sh){return p.then(function(){return syncStatus(sh).catch(function(){return false})})},Promise.resolve(false))};
window.ExportHUBPickupQR={version:'RC535',engine:'single-server-bound',ensure:ensure,register:register,syncStatus:syncStatus,diagnostics:function(){return {singleEngine:true,legacyActivationAliases:false,registeredPinLocked:credentialLocked(current()),credentialVersion:credentialVersion(current()),pinRevision:Number(current().pickupPinRevision||0)}}};
