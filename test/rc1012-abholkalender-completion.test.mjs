...
test('RC1012: Wochenansicht kann exakt eine Woche vor und zurück verschoben werden', () => {
  const next = calendar.buildCalendarModel({
    ...
    shipments:[{id:'S-NEXT',reference:'NX1012',plannedPickupDate:'2026-09-16'}]
  });
  ...
  assert.equal(next.days[2].shipments[0].reference,'NX1012');
...
test('RC1012: Heute bleibt beim Blättern der echte heutige Tag', () => {
...
test('RC1012: View-State und Oberfläche besitzen vollständige Wochennavigation', () => {
...
test('RC1012: SENDUNG-Karte bietet direkten Öffnen-Weg über stabile Identität', () => {
...
test('RC1012: Kalender nutzt die echte ExportHUB-openShipment-Runtime', () => {
  const js = fs.readFileSync('assets/abholkalender.js','utf8');
