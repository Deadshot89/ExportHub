...
module.exports=async function(context,req){
 ...
 try{
  ...
  const expected=count(b.expectedColliCount||b.totalColli||b.colliCount||b.packageCount);if(!expected)throw store.err('COLLI_REQUIRED','Die Soll-Colli-Anzahl fehlt. Bitte die Sendung mit vollständigen Colli-Daten speichern.',400);
  const c=await store.clients(),blob=store.recordBlob(c.records,token);let current=await store.readJson(blob),existing=current.value||{},record=Object.assign({},existing),spedition=carrier(b,existing);
  if((b.resetPickup===true||b.reactivate===true)&&existing){record=clearConfirmation(record)}
  record.token=token;record.reference=text(b.reference||b.shipmentRef||existing.reference);record.shipmentId=text(b.shipmentId||existing.shipmentId);record.customer=text(b.customerName||b.customer||b.recipientCustomerName||existing.customer);record.recipient=text(b.recipient||b.recipientName||existing.recipient);record.address=text(b.recipientAddress||b.deliveryAddress||b.shipToAddress||b.address||existing.address);record.locationName=text(b.locationName||existing.locationName);record.palletOut=Math.max(0,Number(b.palletOut||b.euroPallets||existing.palletOut||0)||0);record.expectedColliCount=expected;record.colliCount=expected;record.totalColli=expected;record.packageCount=expected;if(spedition){record.carrierName=spedition;record.speditionName=spedition;record.carrier=spedition;record.spedition=spedition}record.pinHash=store.hash(pins.bridgePin());record.disabled=Boolean(b.disabled===true||b.active===false);record.createdAt=existing.createdAt||store.now();record.updatedAt=store.now();record.expiresAt=existing.expiresAt||new Date(Date.now()+Math.max(1,Number(b.expiresDays||180))*86400000).toISOString();record.registrationVersion='RC625';record.metadataVersion=14;if(!record.status)record.status='open';
 ...
 }catch(e){context.log&&context.log.error&&context.log.error('pickup-init RC625',e&&e.code,e&&e.message);context.res=store.json(e.status||500,{ok:false,code:e.code||'INIT_FAILED',message:e.message||'QR-Code konnte nicht registriert werden.'})}
};
