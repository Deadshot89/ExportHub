...
const OUT=path.join(ROOT,'dist-rc1041');
...
if(!probe.includes(`__EXPORTHUB_PRODUCTION_VERSION_PROBE__='${VERSION}'`))throw new Error('RC1041 Produktionsmarker fehlt');
fs.writeFileSync(probeFile,probe);

const baseManifest=JSON.parse(fs.readFileSync(path.join(SRC,'rc1018-manifest.json'),'utf8'));
const manifest={
  schema:'exporthub-rc1041-three-env-v1',
  version:VERSION,
...
fs.writeFileSync(path.join(OUT,'rc1041-manifest.json'),JSON.stringify(manifest,null,2)+'\n');
console.log('RC1041 build ready: RC1018-Basis vollständig übernommen, sichtbare Releasekennung auf RC1041 synchronisiert.');
