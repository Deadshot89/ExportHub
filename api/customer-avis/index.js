...
function addAppointmentNotification(state,target,event){if(!event||!obj(state)||!obj(target))return;const after=event.details&&event.details.after||{},reference=sref(target),customer=text(target.customerName||(target.customer&&target.customer.name))||apiI18n.tLang('de','api.avis.customer'),id='avis-appointment-'+event.id,list=arr(state.notifications).slice();if(list.some(x=>obj(x)&&text(x.id)===id))return;const vars={customer,reference,date:text(after.date),from:text(after.timeFrom),to:text(after.timeTo)},title=apiI18n.tLang('de','api.avis.appointmentTitle'),message=apiI18n.tLang('de','api.avis.appointmentMessage',vars);list.push({id,type:'customer-avis-appointment',source:'customer-avis',title,message,_localizedText:{title:localizedMeta('api.avis.appointmentTitle'),message:localizedMeta('api.avis.appointmentMessage',vars)},createdAt:event.at,read:false,route:'notifications',shipmentId:sid(target),shipmentRef:reference,customerName:customer,pickupDate:text(after.date),timeFrom:text(after.timeFrom),timeTo:text(after.timeTo)});state.notifications=list.slice(-200)}
...
async function saveAppointment(blob,sessionInfo,session,payload,firstRead,language){for(let i=0;i<MAX_RETRIES;i++){const d=i===0&&firstRead?firstRead:await readTeam(blob),team=d.value||{},state=obj(team.state)?team.state:{},target=findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference)||((sessionInfo.record&&obj(sessionInfo.record.snapshot))?ensureDraftShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference,sessionInfo.record.snapshot):null);if(!target)throw error('SHIPMENT_NOT_FOUND','Sendung wurde nicht gefunden.',404);if(dateTimeOf(target))throw error('AVIS_CLOSED','api.avis.closedChanges',410);const checked=validateAppointment(payload),slot=avisSlots.slotState(shipmentCopies(state),checked.date,checked.from,checked.to,{subjectId:sid(target),reference:sref(target)});if(!slot)throw error('PICKUP_SLOT_INVALID','Bitte ein freies 2-Stunden-Zeitfenster zwischen 08:30 und 16:00 auswählen.',400);if(!slot.available)throw error('PICKUP_SLOT_FULL','Dieses Zeitfenster ist inzwischen ausgebucht. Bitte wählen Sie einen anderen freien Slot.',409);applyAppointment(state,target,payload);team.state=state;team.revision=Number(team.revision||0)+1;team.updatedAt=now();team.updatedBy='Kunden-Avis';team.updatedByUserId='customer-avis';team.clientVersion='RC1224';try{await writeTeam(blob,team,d.etag);return publicShipment(findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference),session,state,language)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(80+i*100);continue}throw e}}throw error('STATE_CONFLICT','Avis-Bestätigung konnte nicht gespeichert werden.',409)}
