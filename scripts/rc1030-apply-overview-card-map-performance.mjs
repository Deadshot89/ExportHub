...
function replaceOnce(before,after,label){
  ...
  if(first<0)throw new Error(`RC1030 Patchanker fehlt: ${label}`);
  if(source.indexOf(before,first+before.length)>=0)throw new Error(`RC1030 Patchanker nicht eindeutig: ${label}`);
...
console.log('RC1030 Overview-Kartenmap-Patch angewendet.');
