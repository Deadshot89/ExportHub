...
module.exports=async function(context,req){
  try{
    const token=String(req.query&&req.query.token||'').toLowerCase(),got=await store.getRecord(token);
  ...
  }catch(e){context.res=store.json(e.status||500,{ok:false,code:e.code||'SERVER_ERROR',message:e.message||'Status konnte nicht gelesen werden.'})}
};
