...
const dynamicI18n=require('../shared/dynamic-i18n');
...
function localizedShipmentRemark(sh,language){for(const key of ['remark','remarks','bemerkung','comments','comment','note','notes','shipmentRemark']){if(text(sh&&sh[key]))return dynamicI18n.localizedValue(sh,key,language)||text(sh[key])}return''}
...
async function ensureTeamContainerReady(){if(teamContainerReadyPromise)return teamContainerReadyPromise;const cs=connectionString();if(!cs)throw error('STORAGE_NOT_CONFIGURED','api.common.storageNotConfigured',503);teamContainer=BlobServiceClient.fromConnectionString(cs).getContainerClient(TEAM_CONTAINER);teamContainerReadyPromise=Promise.resolve(teamContainer);return teamContainerReadyPromise}
...
async function ensureDocumentContainerReady(){if(documentContainer)return documentContainer;const cs=connectionString();if(!cs)throw error('STORAGE_NOT_CONFIGURED','api.common.storageNotConfigured',503);documentContainer=BlobServiceClient.fromConnectionString(cs).getContainerClient(DOCUMENT_CONTAINER);return documentContainer}
async function ensureQuarantineContainerReady(){if(quarantineContainerReadyPromise)return quarantineContainerReadyPromise;const cs=connectionString();if(!cs)throw error('STORAGE_NOT_CONFIGURED','api.common.storageNotConfigured',503);quarantineContainer=BlobServiceClient.fromConnectionString(cs).getContainerClient(AVIS_QUARANTINE_CONTAINER);quarantineContainerReadyPromise=Promise.resolve().then(async()=>{if(typeof quarantineContainer.createIfNotExists==='function')await quarantineContainer.createIfNotExists();return quarantineContainer}).catch(e=>{quarantineContainerReadyPromise=null;throw e});return quarantineContainerReadyPromise}
...
function sanitizeDraftSnapshot(raw,subjectId,reference){
 ...
 if(!/^[A-Z0-9]{6}$/.test(ref))throw error('REFERENCE_INVALID','api.avis.referenceInvalid',400);
...
function ensureDraftShipment(state,subjectId,reference,snapshot){
 ...
 if(!name)throw error('CUSTOMER_REQUIRED','api.avis.customerRequired',400);
 if(!location)throw error('LOCATION_REQUIRED','api.avis.locationRequired',400);
...
function uploadMessageKey(entry){
 const explicit=text(entry&&entry.messageKey);if(explicit)return explicit;
 const value=text(entry&&entry.message);
 const legacy=['api.avis.scanSaved','api.avis.malicious','api.avis.notScanned','api.avis.scanFailed','api.avis.notSaved','api.avis.scanning'];
...
function addCustomerUploadNotification(state,sh,file,entry){
 ...
 const reference=sref(sh),customer=text(sh.customerName||(sh.customer&&sh.customer.name))||apiI18n.tLang('de','api.avis.customer'),documentName=fileName(file,apiI18n.tLang('de','api.avis.customerDocument')),createdAt=text(entry&&entry.completedAt)||text(file.uploadedAt)||now();
 const title=apiI18n.tLang('de','api.avis.newDocument'),messageVars={customer,document:documentName,reference},message=apiI18n.tLang('de','api.avis.uploadMessage',messageVars);
 const notice={id,type:'customer-avis-document',source:'customer-avis-upload',title,message,_localizedText:{title:localizedMeta('api.avis.newDocument'),message:localizedMeta('api.avis.uploadMessage',messageVars)},createdAt,read:false,route:'notifications',shipmentId:sid(sh),shipmentRef:reference,customerName:customer,documentId:text(file.id),documentName,documentSha256:text(file.sha256),documentBlobName:text(file.blobName),documentMimeType:text(file.mimeType||file.type)||'application/pdf'};
...
async function savePendingCustomerUpload(teamBlob,sessionInfo,session,entry,firstRead,language){
 for(let i=0;i<MAX_RETRIES;i++){
  ...
  if(dateTimeOf(target))throw error('AVIS_CLOSED','api.avis.closed',410);
  ...
  try{await writeTeam(teamBlob,team,d.etag);return publicShipment(findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference),session,state,language)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(80+i*100);continue}throw e}
 }
 throw error('STATE_CONFLICT','api.avis.queueConflict',409)
}
async function updateCustomerUploadOutcome(teamBlob,sessionInfo,session,entry,file,language){
 for(let i=0;i<MAX_RETRIES;i++){
  ...
  if(!copies.length)throw error('SHIPMENT_NOT_FOUND','api.avis.shipmentNotFound',404);
  copies.forEach(sh=>{
 ...
 throw error('STATE_CONFLICT','api.avis.resultConflict',409)
}
async function promoteCleanCustomerPdf(teamBlob,sessionInfo,session,quarantineBlob,uploadId,scan,shipmentForCheck,queueEntry,language){
 ...
 if(validated.sha256!==uploadId)throw error('PDF_HASH_MISMATCH','api.avis.pdfHashMismatch',409);
...
async function blockCustomerPdf(teamBlob,sessionInfo,session,quarantineBlob,uploadId,scan,queueEntry,language){
 const code=scan.status==='malicious'?'PDF_MALWARE_DETECTED':scan.status==='not-scanned'?'PDF_NOT_SCANNED':'PDF_SCAN_FAILED';
 const messageKey=scan.status==='malicious'?'api.avis.malicious':scan.status==='not-scanned'?'api.avis.notScanned':'api.avis.scanFailed',message=apiI18n.tLang('de',messageKey);
 try{if(typeof quarantineBlob.deleteIfExists==='function')await quarantineBlob.deleteIfExists()}catch(_){}
 const shipment=await updateCustomerUploadOutcome(teamBlob,sessionInfo,session,{id:uploadId,name:text(queueEntry&&queueEntry.name)||apiI18n.tLang('de','api.avis.customerDocument'),size:Number(queueEntry&&queueEntry.size||0)||0,status:'blocked',uploadedAt:text(queueEntry&&queueEntry.uploadedAt),completedAt:now(),message,messageKey,scanResult:text(scan.result),scanTime:text(scan.scanTime)},null,language);
 return{ok:false,status:'blocked',code,message:apiI18n.tLang(language,messageKey),upload:{id:uploadId,status:'blocked',scanResult:text(scan.result),scanTime:text(scan.scanTime)},shipment}
}
async function customerPdfScanStatus(teamBlob,sessionInfo,session,sh,uploadId,state,language){
 ...
 if(scan.status==='clean')return promoteCleanCustomerPdf(teamBlob,sessionInfo,session,qBlob,uploadId,scan,sh,queueEntry,language);
 if(scan.status==='malicious'||scan.status==='not-scanned'||scan.status==='error')return blockCustomerPdf(teamBlob,sessionInfo,session,qBlob,uploadId,scan,queueEntry,language);
 ...
 if(ageMs>30*60*1000)return blockCustomerPdf(teamBlob,sessionInfo,session,qBlob,uploadId,{status:'error',result:'Scan timeout',scanTime:''},queueEntry,language);
 return{ok:true,status:'scanning',upload:{id:uploadId,name:text(queueEntry&&queueEntry.name)||apiI18n.tLang('de','api.avis.customerDocument'),size:Number(queueEntry&&queueEntry.size||0)||0,status:'scanning',provider:'Microsoft Defender for Storage'},shipment:publicShipment(sh,session,state,language)}
}
function publicShipment(sh,session,state,language='de'){
 ...
 if(actual)return{ok:true,closed:true,status:'Abgeholt',reference:sref(sh),customerName:text(sh.customerName||(sh.customer&&sh.customer.name)),remark:localizedShipmentRemark(sh,language),totals:{count:totals.count},actualPickupAt:actual,pickup:{carrier:pickupCarrierOf(sh),licensePlate:pickupPlateOf(sh),driverName:pickupDriverOf(sh)},documents:docs,customerUploads:publicCustomerUploads(sh,language),pod:{available:podAvailable(sh),status:text(sh.podStatus)||(podAvailable(sh)?'POD vorhanden':'Noch nicht vorhanden'),documents:podDocs},avis:{enabled:true,closed:true,singleUse:false,session:true,autoExpiresOn,postPickupDays:14},lastUpdatedAt:text(sh.updatedAt||sh._syncUpdatedAt||sh.modifiedAt)};
...
function appointmentLabel(first){return apiI18n.tLang('de',first?'api.avis.appointmentCreated':'api.avis.appointmentChanged')}

function addAppointmentNotification(state,target,event){
 if(!event||!obj(state)||!obj(target))return;
 const after=event.details&&event.details.after||{},reference=sref(target),customer=text(target.customerName||(target.customer&&target.customer.name))||apiI18n.tLang('de','api.avis.customer'),id='avis-appointment-'+event.id,list=arr(state.notifications).slice();
 if(list.some(x=>obj(x)&&text(x.id)===id))return;
 const vars={customer,reference,date:text(after.date),from:text(after.timeFrom),to:text(after.timeTo)},title=apiI18n.tLang('de','api.avis.appointmentTitle'),message=apiI18n.tLang('de','api.avis.appointmentMessage',vars);
 list.push({id,type:'customer-avis-appointment',source:'customer-avis',title,message,_localizedText:{title:localizedMeta('api.avis.appointmentTitle'),message:localizedMeta('api.avis.appointmentMessage',vars)},createdAt:event.at,read:false,route:'notifications',shipmentId:sid(target),shipmentRef:reference,customerName:customer,pickupDate:text(after.date),timeFrom:text(after.timeFrom),timeTo:text(after.timeTo)});
 state.notifications=list.slice(-200)
}
...
function applyAppointment(state,target,payload){const v=validateAppointment(payload),stamp=now(),reference=sref(target),prior=appointmentSnapshot(target),first=!text(target&&(target.customerAvisResponseAt||target.avisResponseAt)),before=first?{date:'',timeFrom:'',timeTo:'',plate:'',note:''}:prior,after={date:v.date,timeFrom:v.from,timeTo:v.to,plate:v.plate,note:v.note},changed=first||!sameAppointment(before,after),values={customerAvisPickupDate:v.date,avisPickupDate:v.date,customerAvisPickupTimeFrom:v.from,avisPickupTimeFrom:v.from,customerAvisPickupTimeTo:v.to,avisPickupTimeTo:v.to,customerAvisPickupPlate:v.plate,avisPickupPlate:v.plate,customerAvisShipmentNumber:reference,avisShipmentNumber:reference,customerAvisPickupNote:v.note,avisPickupNote:v.note,customerAvisResponseAt:stamp,avisResponseAt:stamp,customerAvisResponseReference:reference,avisResponseReference:reference,customerAvisResponseStatus:AVIS_CONFIRMED_STATUS,avisResponseStatus:AVIS_CONFIRMED_STATUS,customerConfirmed:true,customerConfirmedAt:stamp,customerConfirmedVia:'customer-avis',plannedPickupDate:v.date,pickupDate:v.date,updatedAt:stamp,_syncUpdatedAt:stamp},event=changed?{id:'AVIS-APPT-'+crypto.randomBytes(10).toString('hex'),at:stamp,type:'avis',label:appointmentLabel(first),actor:{name:'Kunden-Avis',role:'Kunde'},details:{reference,before,after,oldDate:before.date,newDate:after.date,oldTimeFrom:before.timeFrom,oldTimeTo:before.timeTo,newTimeFrom:after.timeFrom,newTimeTo:after.timeTo,oldPlate:before.plate,newPlate:after.plate}}:null;findCopies(state,sid(target),reference).forEach(sh=>{Object.assign(sh,values);if(event)appendAppointmentHistory(sh,event)});if(event)addAppointmentNotification(state,target,event);return{values,event}}
async function setAvisFlags(blob,subjectId,reference,enabled,actor,firstRead){for(let i=0;i<MAX_RETRIES;i++){const d=i===0&&firstRead?firstRead:await readTeam(blob),team=d.value||{},state=obj(team.state)?team.state:{},copies=findCopies(state,subjectId,reference);if(!copies.length)throw error('SHIPMENT_NOT_FOUND','api.avis.shipmentNotFound',404);const stamp=now(),values=enabled?{customerAvisEnabled:true,avisEnabled:true,customerAvisEnabledAt:stamp,avisEnabledAt:stamp,customerAvisSecurityVersion:1013,avisSecurityVersion:1013,customerAvisDisabledAt:'',avisDisabledAt:'',updatedAt:stamp,_syncUpdatedAt:stamp}:{customerAvisEnabled:false,avisEnabled:false,customerAvisDisabledAt:stamp,avisDisabledAt:stamp,customerAvisSecurityVersion:0,avisSecurityVersion:0,updatedAt:stamp,_syncUpdatedAt:stamp};copies.forEach(sh=>Object.assign(sh,values));team.state=state;team.revision=Number(team.revision||0)+1;team.updatedAt=stamp;team.updatedBy=actor||'Kunden-Avis';team.updatedByUserId='customer-avis';team.clientVersion='RC1129';try{await writeTeam(blob,team,d.etag);return findShipment(state,subjectId,reference)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(60+i*80);continue}throw e}}throw error('STATE_CONFLICT','api.avis.flagSaveFailed',409)}
async function saveAppointment(blob,sessionInfo,session,payload,firstRead,language){for(let i=0;i<MAX_RETRIES;i++){const d=i===0&&firstRead?firstRead:await readTeam(blob),team=d.value||{},state=obj(team.state)?team.state:{},target=findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference)||((sessionInfo.record&&obj(sessionInfo.record.snapshot))?ensureDraftShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference,sessionInfo.record.snapshot):null);if(!target)throw error('SHIPMENT_NOT_FOUND','api.avis.shipmentNotFound',404);if(dateTimeOf(target))throw error('AVIS_CLOSED','api.avis.closedChanges',410);const checked=validateAppointment(payload),slot=avisSlots.slotState(shipmentCopies(state),checked.date,checked.from,checked.to,{subjectId:sid(target),reference:sref(target)});if(!slot)throw error('PICKUP_SLOT_INVALID','api.avis.slotInvalid',400);if(!slot.available)throw error('PICKUP_SLOT_FULL','api.avis.slotFull',409);applyAppointment(state,target,payload);team.state=state;team.revision=Number(team.revision||0)+1;team.updatedAt=now();team.updatedBy='Kunden-Avis';team.updatedByUserId='customer-avis';team.clientVersion='RC1224';try{await writeTeam(blob,team,d.etag);return publicShipment(findShipment(state,sessionInfo.record.subjectId,sessionInfo.record.reference),session,state,language)}catch(e){if(isConflict(e)&&i<MAX_RETRIES-1){await wait(80+i*100);continue}throw e}}throw error('STATE_CONFLICT','api.avis.saveFailed',409)}
 ...
 try{
  ...
  if(req.method==='POST'){
   ...
   if(postAction==='upload-document'){
    ...
    try{
     const shipment=await savePendingCustomerUpload(blob,sessionInfo,session,{id:validated.sha256,name:validated.name,size:validated.size,status:'scanning',documentType:docType,uploadedAt:quarantine.uploadedAt,message:apiI18n.tLang('de','api.avis.scanning'),messageKey:'api.avis.scanning'},d,language);
   ...
   if(postAction==='document-upload-status'){
    context.res=json(200,await customerPdfScanStatus(blob,sessionInfo,session,sh,lower(payload.uploadId),state,language));return
   }
   if(postAction!=='appointment')throw error('UNKNOWN_ACTION','api.avis.unknownAction',400);
   context.res=json(200,await saveAppointment(blob,sessionInfo,session,payload,d,language));return
  }
  context.res=json(405,{ok:false,code:'METHOD_NOT_ALLOWED'},{Allow:'GET, POST, OPTIONS'});
 }catch(e){context.log&&context.log.error&&context.log.error('customer-avis RC1224',e&&e.code,e&&e.message);context.res=json(e.status||e.statusCode||500,{ok:false,code:e.code||'SERVER_ERROR',message:localizedError(req,e,'api.avis.unavailable')})}
};