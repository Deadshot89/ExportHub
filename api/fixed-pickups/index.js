...
module.exports = async function(context, req){
  ...
  if (method === 'OPTIONS') {
    context.res = auth.json(204, {});
    return;
  }
  try {
    if (!['GET','POST','PATCH'].includes(method)) {
      throw auth.error('METHOD_NOT_ALLOWED', 'Methode nicht erlaubt.', 405);
    }
    ...
    if (!admin) {
      throw auth.error('ADMIN_REQUIRED', 'Nur Administratoren dürfen fixe Abholungen ändern.', 403);
    }
    ...
    context.res = auth.json(e && (e.status || e.statusCode) || 500, {
      ok: false,
      code: e && e.code || 'FIXED_PICKUPS_FAILED',
      message: e && e.message || 'Fixe Abholungen konnten nicht verarbeitet werden.'
    });
  }
};
