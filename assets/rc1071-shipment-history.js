...
function monitor(){
 ...
 fileDiff(sh,prev.abdFiles,next.abdFiles,'abd',de('shipmentHistory.document.abdDocument'));
 fileDiff(sh,prev.podFiles,next.podFiles,'pod',de('shipmentHistory.document.pod'));
 fileDiff(sh,prev.deliveryFiles,next.deliveryFiles,'document',de('shipmentHistory.document.shippingDocument'));
...
function documentLabel(text){text=low(text);if(/\babd\b|ausfuhrbegleit/.test(text))return'ABD';if(/\bpod\b|proof of delivery|abliefer/.test(text))return'POD';if(/cmr/.test(text))return'CMR';if(/rechnung|invoice/.test(text))return de('shipmentHistory.document.invoice');if(/lieferschein|liefer.?schein|delivery note/.test(text))return de('shipmentHistory.document.deliveryNote');if(/deckblatt/.test(text))return de('shipmentHistory.document.coverSheet');if(/stauplan/.test(text))return de('shipmentHistory.document.stowagePlan');if(/ladeliste/.test(text))return de('shipmentHistory.document.loadingList');if(/gesamt/.test(text)&&/druck|ausgabe|pdf/.test(text))return de('shipmentHistory.document.totalPrint');if(/l1/.test(text)&&/qr/.test(text))return'L1 QR';if(/\bl2\b/.test(text))return'L2';if(/pdf/.test(text))return'PDF';return de('shipmentHistory.document.generic')}
...
function documentFilesFor(sh,doc){
 var keys=doc==='ABD'?['abdFiles']:doc==='POD'?['podFiles']:doc===de('shipmentHistory.document.invoice')?['invoiceFiles']:doc===de('shipmentHistory.document.deliveryNote')?['deliveryFiles','deliveryNotesFiles','lieferscheine']:[];
...
function mailTypeFrom(text){var l=low(text);if(/\babd\b|ausfuhrbegleit/.test(l))return de('shipmentHistory.mail.abdRequest');if(/anmeld|registrier|versandbestät/.test(l))return de('shipmentHistory.mail.registration');if(/lieferavis|abholung|collection notice/.test(l))return de('shipmentHistory.mail.avis');return de('shipmentHistory.mail.email')}
