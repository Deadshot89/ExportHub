...
async function promoteCleanCustomerPdf(teamBlob,sessionInfo,session,quarantineBlob,uploadId,scan,shipmentForCheck,queueEntry){
 ...
 const completedEntry={id:uploadId,name,size:bytes.length,status:'saved',documentType:docType,uploadedAt:text(queueEntry&&queueEntry.uploadedAt)||text(metadata.uploadedat),completedAt:uploadedAt,message:apiI18n.tLang('de','api.avis.scanSaved'),scanResult:text(scan.result),scanTime:text(scan.scanTime),contentCode:content.code,contentMatched:content.matched};
...
async function blockCustomerPdf(teamBlob,sessionInfo,session,quarantineBlob,uploadId,scan,queueEntry){
 ...
 const message=apiI18n.tLang('de',scan.status==='malicious'?'api.avis.malicious':scan.status==='not-scanned'?'api.avis.notScanned':'api.avis.scanFailed');
 try{if(typeof quarantineBlob.deleteIfExists==='function')await quarantineBlob.deleteIfExists()}catch(_){}
...
async function customerPdfScanStatus(teamBlob,sessionInfo,session,sh,uploadId,state){
 ...
 const saved=customerUploadFile(sh,uploadId);if(saved)return{ok:true,status:'saved',upload:{id:uploadId,name:fileName(saved,apiI18n.tLang('de','api.avis.customerDocument')),size:Number(saved.size||0)||0,status:'saved'},shipment:publicShipment(sh,session,state)};
 const queueEntry=customerUploadQueueEntry(sh,uploadId);if(queueEntry&&text(queueEntry.status)==='blocked')return{ok:false,status:'blocked',code:'PDF_SCAN_BLOCKED',message:(()=>{const raw=text(queueEntry.message);return /^api\./.test(raw)?apiI18n.t(req,raw):raw||apiI18n.t(req,'api.avis.notSaved')})(),upload:queueEntry,shipment:publicShipment(sh,session,state)};
 ...
 return{ok:true,status:'scanning',upload:{id:uploadId,name:text(queueEntry&&queueEntry.name)||apiI18n.tLang('de','api.avis.customerDocument'),size:Number(queueEntry&&queueEntry.size||0)||0,status:'scanning',provider:'Microsoft Defender for Storage'},shipment:publicShipment(sh,session,state)}
}
 ...
 try{
  ...
  if(req.method==='POST'){
   ...
   if(postAction==='upload-document'){
    ...
    try{
     const shipment=await savePendingCustomerUpload(blob,sessionInfo,session,{id:validated.sha256,name:validated.name,size:validated.size,status:'scanning',documentType:docType,uploadedAt:quarantine.uploadedAt,message:apiI18n.tLang('de','api.avis.scanning')},d);
