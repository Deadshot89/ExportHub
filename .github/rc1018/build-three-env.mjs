...
function patchGate41NationalOnly(html){
  ...
  for(const [before,after] of replacements)if(out.includes(before))out=out.replace(before,after);
  ...
  for(const marker of required)if(!out.includes(marker))throw new Error('RC1044 Gate41 National-Only fehlt: '+marker);
  return out
}
