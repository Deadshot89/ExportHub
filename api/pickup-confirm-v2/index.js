...
module.exports=async function(context,req){
 ...
 try{
  ...
  let rec=await store.mutateRecord(accessKey,resolved.environment,async function(r,clients){
   ...
   const expected=store.expectedCollis(r),remainingBefore=remainingOf(r),entered=count(store.first(b,['enteredColliCount','colliCount','pickupColliCount'])),mode=String(b.pickupMode||b.mode||'complete').toLowerCase();if(!expected)throw store.err('COLLI_EXPECTED_MISSING','Die Soll-Colli-Anzahl fehlt im QR-Datensatz. Bitte den QR-Code in ExportHUB neu erzeugen.',409);if(!entered)throw store.err('COLLI_REQUIRED','Bitte die auf diesem LKW verladenen Colli eingeben.',400);if(entered>remainingBefore)throw store.err('COLLI_EXCEEDS_REMAINING','Es können nicht mehr Colli bestätigt werden als noch offen sind.',409);if(mode==='complete'&&entered!==remainingBefore)throw store.err('COLLI_MISMATCH','Für eine vollständige Abholung muss die gesamte Restmenge bestätigt werden.',409);
   ...
   if(mode==='complete'&&containerRequired&&!sealNumber)throw store.err('CONTAINER_SEAL_REQUIRED','Für diese Seefracht-Sendung ist die Siegelnummer verpflichtend.',409);
   if(mode==='complete'&&containerRequired&&!containerDocs.completePhotos(containerPhotos))throw store.err('CONTAINER_PHOTOS_REQUIRED','Für diese Seefracht-Sendung müssen alle 3 Containerfotos gespeichert sein.',409);
 ...
 }catch(e){context.log&&context.log.error&&context.log.error('pickup-confirm-v2 RC1259',e&&e.code,e&&e.message);context.res=json(e.status||e.statusCode||500,{ok:false,code:e.code||'SERVER_ERROR',message:e.message||'Abholung konnte nicht bestätigt werden.'})}
};
