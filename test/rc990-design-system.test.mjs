...
test('RC990 Design: aktive RC229-Aufgaben brauchen keinen nicht gesetzten Body-View-Marker',()=>{
  assert.match(style,/#content\s+\.rc229-task-grid\s*\{[\s\S]{0,300}grid-template-columns\s*:\s*repeat\(3\s*,\s*minmax\(0\s*,\s*1fr\)\)/i,'aktives RC229-Raster muss direkt über #content getroffen werden');
  assert.match(style,/#content\s+\.rc229-task-card\.rc628-unified-task\s*\{/i,'aktive RC628-Aufgabenkarten müssen direkt über #content gestaltet werden');
});
