...
const CALENDAR_RUNTIME='/assets/rc1012-abholkalender-runtime.js?v=1012&rc=1030';
const DIAGNOSTICS_SRC='/assets/rc1013-diagnostics.js?v=1013';
const GATE41_UI_SRC='/assets/rc1013-gate41-ui.js?v=1013';
const LIEFERAVIS_SRC='/assets/rc1015-lieferavis-mail-flow.js?v=1291';
...
function multiTruckTag(){return `<script id="exporthub-rc1017-multi-truck" defer src="${MULTI_TRUCK_SRC}"><\/script>`;}
function diagnosticTags(){return `<script id="exporthub-rc1013-diagnostics" defer src="${DIAGNOSTICS_SRC}"><\/script>\n<script id="exporthub-rc1013-gate41-ui" defer src="${GATE41_UI_SRC}"><\/script>\n<script id="exporthub-rc1015-lieferavis-mail-flow" defer src="${LIEFERAVIS_SRC}"><\/script>`;}
...
copy('assets/abholkalender.js');copy('assets/abholkalender.css');copy('assets/rc1012-abholkalender-runtime.js');copy('assets/rc1013-diagnostics.js');copy('assets/rc1013-gate41-ui.js');copy('assets/rc1015-lieferavis-mail-flow.js');copy('assets/rc1017-multi-truck.js');
write('rc1013-manifest.json',JSON.stringify({schema:'exporthub-rc1013-three-env-v1',version:VERSION,generatedAt:new Date().toISOString(),releaseEnvironments:RELEASE_ENVIRONMENTS,sopRelease:'assets/sop/rc1010-sop-release.js',sopStability:'assets/sop/rc1013-sop-stability.js',calendar:{core:'assets/abholkalender.js',style:'assets/abholkalender.css',runtime:'assets/rc1012-abholkalender-runtime.js'},fixes:{gate41:'Deutschland-Fallback und sichtbarer Preisstatus',pickup:'Server-Token wird nach QR-Registrierung synchronisiert; abgeschlossener Status bleibt lesbar',diagnostics:'strukturierte Fehlercodes und Erklärungen',sop:'Filter-Render wird aus laufenden DOM-Events entkoppelt',storage:'transiente Azure-Fehler werden wiederholt',android:'native Direktansicht',lieferavis:'Aktivierung speichert die Sendung automatisch; Kundenmail wechselt eindeutig zwischen Sendungsdetails und Lieferavis'},environments:{production:{file:'index.html',sha256:sha(production)},testservice:{file:'TESTVERSION.html',sha256:sha(testservice)},demo:{file:'demo.html',sha256:sha(demo)}}},null,2)+'\n');
console.log('RC1013 build ready: Diagnoseursachen, Gate41, QR-Abholung, Lieferavis-Mailfluss, SOP-Stabilität und Storage-Resilienz in Produktion, TESTSERVICE und Demo');
