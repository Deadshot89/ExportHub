  ...
  try {
    ...
    if (metadata.revision !== undefined) {
      return {
        ...
        updatedAt: metadata.updatedepoch
          ? new Date(Number(metadata.updatedepoch)).toISOString()
...
async function uploadTeam(blob, value, etag) {
  ...
  return blob.upload(body, Buffer.byteLength(body), {
    ...
    metadata: {
      ...
      clientversion: String(value.clientVersion || '')
...
async function saveMerged(blob, incoming, sessionUser) {
  for (let attempt = 0; attempt < MAX_RETRIES; attempt += 1) {
    ...
    const mergedState = pruneTombstones(
    ...
    const next = {
      ...
      users: current.users || [],
      authBootstrap:
        current.authBootstrap && typeof current.authBootstrap === 'object'
    ...
    try {
      ...
      next.concurrentMerge =
        Number(incoming.baseRevision || 0) !== Number(current.revision || 0);
      next.baseRevision = Number(incoming.baseRevision || 0);
      return next;
    } catch (error) {
      if (error && error.statusCode === 412 && attempt < MAX_RETRIES - 1) {
        continue;
      }
      throw error;
    }
  ...
  throw auth.error(
    'CONCURRENT_UPDATE',
    'Der Teamstand konnte nach mehreren Konfliktversuchen nicht gespeichert werden.',
    409
  );
}
  ...
  try {
    ...
    if (payload.sessionToken) {
      req.headers = Object.assign({}, req.headers || {}, {
        'x-exporthub-token': auth.text(payload.sessionToken)
      });
    }
    ...
    if (
      req.method === 'GET' ||
      (req.method === 'POST' && (mode === 'read' || mode === 'meta'))
    ) {
      const metaRequested =
        mode === 'meta' ||
        (req.query && String(req.query.meta || '') === '1');

      if (metaRequested) {
        const meta = await metadataOnly(blob);
        context.res = auth.json(
          200,
          Object.assign({ ok: true, metaOnly: true }, meta)
        );
        return;
      }
      ...
      const clientDocument = auth.sanitizeDocumentForClient(
        document,
        auth.isAdmin(currentSession.user)
      );

      context.res = auth.json(
        200,
        Object.assign({ ok: true }, clientDocument)
      );
      return;
    }
    ...
    if (req.method === 'POST') {
      if (mode && mode !== 'save') {
        throw auth.error(
          ...
          'Unbekannte Teamdatenaktion.',
          400
        );
      }

      if (!auth.hasAnyEditRight(currentSession.user)) {
        throw auth.error(
          'WRITE_FORBIDDEN',
          'Für Änderungen fehlen Bearbeitungsrechte.',
          403
        );
      }
      ...
      const clientSaved = auth.sanitizeDocumentForClient(
        saved,
        auth.isAdmin(currentSession.user)
      );
      ...
      const ackOnly =
        req.query &&
        (
          String(req.query.ack || '') === '1' ||
          String(req.query.mode || '').toLowerCase() === 'ack' ||
          String(req.query.mode || '').toLowerCase() === 'save'
        );
      ...
      context.res = auth.json(
        200,
        Object.assign({ ok: true }, clientSaved)
      );
      return;
    }
