      ...
      const result=await page.evaluate(async timeoutMs=>{
        ...
        try{
          return await Promise.race([
            Promise.resolve(clean.flushSave('RC1154 Browser-Gate wartet auf Azure-Speicherung',{force:true,userInitiated:true}))
              .then(ok=>({ok:ok===true})),
...
export async function openExportHubView(page,module,labels=[],requiredText,options={}){
  ...
  if(responsiveViewport(page)){
    menuOpened=(await menuIsOpen(page))||(await openMenu(page));
  }
