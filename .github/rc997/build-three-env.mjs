...
function setVersion(html){
  ...
  out=out.replace(/var BUILD=Object\.freeze\(\{version:'RC\d+',cache:'\d+',loginReturn:'([^']*)'\}\);/,(_m,ret)=>{
    const next=String(ret||'').replace(/v=\d+/,'v=997');
...
function injectBeforeHeadClose(html,tag,id){
  ...
  if(idx<0)throw new Error('Kein </head> für RC997-Injektion gefunden.');
  return html.slice(0,idx)+tag+'\n'+html.slice(idx);
}
function injectAfterHeadOpen(html,tag,id){
  if(html.includes(`id="${id}"`)||html.includes(`id='${id}'`))return html;
  const m=html.match(/<head(?:\s[^>]*)?>/i);
  if(!m||m.index==null)throw new Error('Kein <head> für RC997-Demo-Injektion gefunden.');
  const idx=m.index+m[0].length;
  return html.slice(0,idx)+'\n'+tag+'\n'+html.slice(idx);
}
function envTag(env){return `<script id="exporthub-rc997-env-config">window.__EXPORTHUB_FORCED_ENVIRONMENT__=${JSON.stringify(env)};<\/script>\n<script id="exporthub-rc997-env-hub" defer src="${HUB_SRC}"><\/script>`;}
function demoTag(){return `<script id="exporthub-rc997-demo-bootstrap" src="${DEMO_SRC}"><\/script>`;}
...
function rc997DemoAsset(){
  return read('assets/exporthub-demo-bootstrap.js')
    .replace(/revision:996/g,'revision:997')
    .replace(/clientVersion:'RC996-demo'/g,"clientVersion:'RC997-demo'")
    .replace(/version:'RC996'/g,"version:'RC997'")
    .replace(/eh996-demo-banner/g,'eh997-demo-banner');
}

function buildTest(){
  let html=setVersion(read('TESTVERSION.html'));
  html=injectBeforeHeadClose(html,envTag('testservice'),'exporthub-rc997-env-config');
  html=ensureMarker(html,'testservice');
  return html;
}
function buildProductionCandidate(){
  let html=setVersion(read('index.html'));
  html=injectBeforeHeadClose(html,envTag('production'),'exporthub-rc997-env-config');
  html=ensureMarker(html,'production-candidate');
  return html;
}
function buildDemo(){
  let html=setVersion(read('TESTVERSION.html'));
  html=injectAfterHeadOpen(html,demoTag(),'exporthub-rc997-demo-bootstrap');
  html=injectBeforeHeadClose(html,envTag('demo'),'exporthub-rc997-env-config');
  html=ensureMarker(html,'demo');
  return html;
}

for(const rel of ['assets/exporthub-environment-hub.js','assets/exporthub-demo-bootstrap.js']){
  if(!fs.existsSync(path.join(ROOT,rel)))throw new Error('RC997 Pflichtdatei fehlt: '+rel);
}
...
write('assets/exporthub-demo-bootstrap.js',rc997DemoAsset());

const manifest={
  schema:'exporthub-rc997-three-env-v1',version:VERSION,generatedAt:new Date().toISOString(),
  source:{test:'TESTVERSION.html',production:'index.html',sourceBaseline:'RC995/RC990'},
  environments:{
...
write('rc997-manifest.json',JSON.stringify(manifest,null,2)+'\n');
console.log(`RC997 build ready: production candidate, TESTSERVICE and Demo -> ${path.relative(ROOT,OUT)}`);
