...
module.exports=async function(context,req){
  ...
  try{
    ...
    if(store.expired(got.record)&&!got.record.confirmedAt)throw store.err('EXPIRED','QR-Code ist abgelaufen.',410);
    context.res=store.json(200,store.publicRecord(got.record));
  }catch(e){
    context.log&&context.log.error&&context.log.error('pickup-status RC626',e&&e.code,e&&e.message);
