...
function insertBeforeLast(text,anchor,addition,label){
  const pos=text.lastIndexOf(anchor);
  if(pos<0) throw new Error(`${label}: Dokumentanker fehlt`);
  return text.slice(0,pos)+addition+text.slice(pos);
}
...
for(const file of htmlFiles){
  ...
  if(!s.includes(`id=\"${marker}\"`)){
    ...
    s=insertBeforeLast(s,'</body>',guard,`${file} Guard-Injektion`);
  }
