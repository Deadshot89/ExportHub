(function(){
'use strict';
const BUILD=window.EXPORTHUB_BUILD||{version:'RC524',cache:'524',loginReturn:'/?v=524'};
const VERSION=String(BUILD.version||'RC524');
...
function rc524StyleHealth(){
 ...
 let link=by('rc521MainStyles');
 if(!link){link=document.createElement('link');link.id='rc521MainStyles';link.rel='stylesheet';document.head.appendChild(link)}
 link.href='assets/exporthub-ui-rc521.css?v='+CACHE+'&retry='+Date.now();
 return fetch('assets/exporthub-ui-rc521.css?v='+CACHE,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('CSS HTTP '+r.status);return r.text()}).then(function(css){
  ...
  return true
 }).catch(function(e){console.error('RC524 Design konnte nicht nachgeladen werden',e);return false})
}
...
async function loadLegacy(){
 ...
 try{if(window.ExportHUBRC524&&typeof window.ExportHUBRC524.install==='function')window.ExportHUBRC524.install()}catch(e){console.error('RC524 Konsolidierung konnte nicht aktiviert werden',e);throw e}
