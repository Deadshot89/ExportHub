(function(){
'use strict';
if(window.__EXPORTHUB_RC542_FINAL_POLICY__) return;
window.__EXPORTHUB_RC542_FINAL_POLICY__=true;
var P={
 version:'RC542', dateFormat:'TT.MM.JJJJ', locale:'de-DE',
...
function save(){try{if(window.ExportHUBClean&&window.ExportHUBClean.queueSave)window.ExportHUBClean.queueSave('RC542 Prozessregeln');}catch(e){}}
function installPolicy(){var s=state();s.settings=s.settings||{};s.settings.finalProcessVersion='RC542';s.settings.dateFormat='TT.MM.JJJJ';s.settings.qrPin=s.settings.qrPin||'2578';s.settings.processPolicy=P;save();}
...
function markVersion(){document.documentElement.setAttribute('data-exporthub-version','RC542');document.querySelectorAll('[data-exporthub-version-label]').forEach(function(n){n.textContent='RC542'})}
...
window.ExportHUBRC542Final={policy:P,postProcess:post};
})();
