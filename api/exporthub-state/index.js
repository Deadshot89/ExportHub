...
async function readTeamForSession(c,requestedMode){
 const read=()=>((requestedMode==='recovery-preview'||requestedMode==='recover-shipments')?readTeamResilient(c.container,c.team,c.teamBlobName,c.recoveryPrefix,c.allowGenericRecoveryDiscovery):readTeamFast(c.team));
...
function validateWriteUserAgainstTeam(team,user,session){const users=Array.isArray(team&&team.users)?team.users:[],fresh=users.find(u=>text(u.id)===text(user&&user.id)||usernameOf(u)===usernameOf(user));if(!fresh||!isActive(fresh))throw error('ACCOUNT_DISABLED','Das Benutzerkonto ist deaktiviert.',403);if(session&&Number(session.authVersion||0)!==Number(fresh.authVersion||0))throw error('SESSION_REVOKED','Die Sitzung wurde beendet. Bitte erneut anmelden.',401);if(isPrivilegedUser(fresh)&&session&&!session.mfaVerifiedAt)throw error('MFA_REAUTH_REQUIRED','Für dieses Administratorkonto ist eine erneute Anmeldung mit zweitem Faktor erforderlich.',401);if((session&&session.mustChange||fresh.mustChange)===true)throw error('PASSWORD_CHANGE_REQUIRED','Vor der Nutzung muss das Startpasswort geändert werden.',403);if(!hasAnyEditRight(fresh))throw error('WRITE_FORBIDDEN','Für Änderungen fehlen Bearbeitungsrechte.',403);return fresh}
