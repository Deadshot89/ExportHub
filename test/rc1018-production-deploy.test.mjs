...
test('RC1045 ist der aktuelle gemeinsame Standarddeploy auf der geprüften RC1044/RC1018-Buildkette',()=>{
  ...
  assert.match(flow,/name:\s*ExportHUB RC1045 Drei-Umgebungen Deploy/);
  assert.match(flow,/node \.github\/rc1045\/build-three-env\.mjs/);
  assert.match(flow,/dist-rc1045\/index\.html/);
  assert.match(flow,/dist-rc1045\/TESTVERSION\.html/);
  assert.match(flow,/dist-rc1045\/demo\.html/);
  assert.match(wrapper,/\.github\/rc1044\/build-three-env\.mjs/,'RC1045 muss den vollständig geprüften RC1044-Vorgänger übernehmen');
  assert.match(wrapper,/dist-rc1044/);
  assert.match(previous,/\.github\/rc1018\/build-three-env\.mjs/,'RC1044 muss weiterhin die geprüfte RC1018-Buildbasis übernehmen');
  assert.match(flow,/Deploy ExportHUB production/);
  assert.match(flow,/Deploy ExportHUB TESTSERVICE/);
});

test('RC1045 bewahrt die RC1018 Mail- und Sprachruntime unverändert',()=>{
...
test('RC1045 Deploypakete und Liveprüfung verwenden überall denselben sichtbaren Release',()=>{
  const flow=read(workflow);
  assert.match(flow,/\.rc1045_production_app/);
  assert.match(flow,/\.rc1045_testservice_app/);
  assert.match(flow,/Live RC1045 Produktion TESTSERVICE und Demo prüfen/);
  assert.match(flow,/ExportHUB RC1045 environment=production-candidate/);
  assert.match(flow,/ExportHUB RC1045 environment=testservice/);
  assert.match(flow,/ExportHUB RC1045 environment=demo/);
  assert.match(flow,/version:'RC1045'/);
  assert.match(flow,/rc1045=\$GITHUB_SHA-\$attempt/);
});
