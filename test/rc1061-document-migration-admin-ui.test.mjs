...
test('RC1061: Dokumentmigration wird nur globalen Admins angeboten',()=>{
  ...
  assert.equal(api.isAdmin({role:'Benutzer'}),false);assert.equal(api.isAdmin({role:'Funktionsadministrator'}),false);assert.equal(api.isAdmin({role:'Benutzer',isAdmin:true,admin:true}),false);assert.equal(api.isAdmin({role:'Globaler Administrator'}),true);assert.equal(api.isAdmin({rights:{settings:{admin:true}}}),false);
});
