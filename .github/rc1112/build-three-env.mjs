...
function patchShipmentSuspendSave(html,file){
  ...
  if(count!==1)throw new Error(file+': RC1154 flushEditSave-Anker '+count+'x gefunden');
  html=html.replace(anchor,replacement);
  if(!html.includes("Sendungseingabe vor Verlassen gespeichert"))throw new Error(file+': RC1154 Shipment-pagehide-Anker fehlt');
  if(!html.includes("!editSaveReason&&!editSaveTimer"))throw new Error(file+': RC1154 No-op Suspend-Save Guard fehlt');
  return html
}
