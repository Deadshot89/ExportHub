...
function applyGate41Fallback(html){
  ...
  if(!out.includes(after)){
    if(!out.includes(before))throw new Error('Gate41 setRouteData-Pfad nicht gefunden.');
    out=out.replace(before,after);
  }
  if(!out.includes('var dc=countryName(d.country)||q(fallbackCountry);')){
    if(!out.includes('var dc=countryName(d.country);'))throw new Error('Gate41 Länderableitung nicht gefunden.');
    out=out.replace('var dc=countryName(d.country);','var dc=countryName(d.country)||q(fallbackCountry);');
  }
  if(!out.includes("setRouteData(u,'destinationCountry');setRouteData(g,'country','Deutschland');")){
    const call="setRouteData(u,'destinationCountry');setRouteData(g,'country');";
    if(!out.includes(call))throw new Error('Gate41 Route-Aufruf nicht gefunden.');
    out=out.replace(call,"setRouteData(u,'destinationCountry');setRouteData(g,'country','Deutschland');");
  }
  return out;
}
...
function applyQrServerToken(html){
  ...
  if(!out.includes(patched)){
    if(!out.includes(base))throw new Error('QR basePatch-Pfad nicht gefunden.');
    out=out.replace(base,patched);
  }
  return out;
}
...
function injectCalendarMenu(html){
  ...
  const arrayStart=html.indexOf('[',itemsAt),arrayEnd=html.indexOf('];',arrayStart);
  if(arrayStart<0||arrayEnd<0)throw new Error('Kanonische ITEMS-Liste unvollständig.');
  const item="{view:'pickupcalendar',label:'Abholkalender',right:'pickupcalendar'}",block=html.slice(arrayStart,arrayEnd),taskAt=block.indexOf("view:'tasks'");
  if(taskAt>=0){const objectEnd=block.indexOf('}',taskAt);if(objectEnd<0)throw new Error('Aufgaben-Menüeintrag unvollständig.');const absolute=arrayStart+objectEnd+1;return html.slice(0,absolute)+','+item+html.slice(absolute);}
  return html.slice(0,arrayEnd)+','+item+html.slice(arrayEnd);
}

function prepare(html,env,isDemo=false){
  let out=setVersion(html);
  out=applyGate41Fallback(out);
  out=applyQrServerToken(out);
  out=injectCalendarMenu(out);
  if(isDemo)out=injectAfterHeadOpen(out,demoTag(),'exporthub-rc1013-demo-bootstrap');
  out=injectBeforeHeadClose(out,sopReleaseTag(),'exporthub-rc1013-sop-release');
  out=injectBeforeHeadClose(out,calendarTags(),'exporthub-rc1013-calendar-css');
  out=injectBeforeHeadClose(out,diagnosticTags(),'exporthub-rc1013-diagnostics');
  out=injectBeforeHeadClose(out,envTag(env),'exporthub-rc1013-env-config');
