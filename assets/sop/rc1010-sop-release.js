...
if(!catalog||!model||!Array.isArray(catalog.documents))return;
...
function releaseDocument(source){
  ...
  const history=releaseHistory(doc);
  const sections=clone(doc.sections||{});
  sections.changeHistory=clone(history);
  const createdBy=text(doc.createdBy)||'ExportHUB – RC1008 SOP-Ersterstellung';
  const createdAt='2026-09-08T12:00:00.000Z';
  const version={
    version:'1.0',status:'Freigegeben',title:text(doc.title),content:clone(sections),visuals:clone(doc.visuals||[]),
    createdBy,createdAt,changeReason:text(doc.changeReason)||'RC1008 – Umstellung auf reine ExportHUB-Systemprozesse',
    submittedBy:REVIEWER,submittedAt:RELEASED_AT,reviewedBy:REVIEWER,reviewedAt:RELEASED_AT,
    approvedBy:APPROVER,approvedAt:RELEASED_AT,validFrom:RELEASE_DATE
  };
  Object.assign(doc,{
    version:'1.0',status:'Freigegeben',validFrom:RELEASE_DATE,reviewedBy:REVIEWER,approvedBy:APPROVER,
    nextReview:NEXT_REVIEW,currentVersion:'1.0',draftVersion:'',updatedAt:RELEASED_AT,sections,history,
    versions:[version],
    auditTrail:[
      {action:'Erstellt',version:'1.0',actor:createdBy,at:createdAt,reason:text(doc.changeReason)||'Ersterstellung'},
      {action:'Zur Prüfung eingereicht',version:'1.0',actor:REVIEWER,at:RELEASED_AT,reason:'RC1010 Qualitätsprüfung'},
...
root.ExportHubIsoSopCatalog=Object.freeze(Object.assign({},catalog,{
  ...
  byNumber:releasedByNumber,
  get(number){return releasedByNumber[text(number)]||null;}
}));

function isPureRc1008SystemDraft(doc){
  if(!doc||!/^SOP-EH-\d{3}$/.test(text(doc.number)))return false;
  if(text(doc.status)!=='Entwurf')return false;
  if(text(doc.approvedBy)||text(doc.reviewedBy))return false;
  ...
  const audit=Array.isArray(doc.auditTrail)?doc.auditTrail:[];
  if(audit.some(item=>text(item&&item.action)!=='Erstellt'))return false;
  return true;
}
...
function reconcileCatalog(storedDocuments,canonicalDocuments){
  ...
  const hadLegacy=stored.some(isPureRc1008SystemDraft);
  const safeStored=hadLegacy?stored.filter(doc=>!isPureRc1008SystemDraft(doc)):stored;
  const reconciled=originalReconcile(safeStored,canonical);
  if(hadLegacy&&Array.isArray(storedDocuments)&&!Object.isFrozen(storedDocuments)){
    storedDocuments.splice(0,storedDocuments.length,...reconciled);
  }
  return reconciled;
}
