// ExportHUB RC1159 – sichere kundenspezifische Portalzugänge.
(function(w,d){
'use strict';
if(!w||!d||w.__EXPORTHUB_RC1159_CUSTOMER_PORTAL__)return;
w.__EXPORTHUB_RC1159_CUSTOMER_PORTAL__=true;
...
function clearRevealedSecrets(){
 ...
 d.querySelectorAll('[data-rc1159-secret]').forEach(function(n){n.textContent='';n.removeAttribute('data-rc1159-secret')});
 var dlg=d.getElementById('rc1159RevealDialog');if(dlg)dlg.remove();
}
function ensureStyle(){
 if(d.getElementById('rc1159CustomerPortalStyle'))return;
 var s=d.createElement('style');s.id='rc1159CustomerPortalStyle';s.textContent=
 '.rc1159-box{margin:16px 0;padding:16px;border:1px solid #cbd5e1;border-radius:14px;background:#fff;color:#0f172a;box-shadow:0 6px 20px rgba(15,23,42,.06)}'+
 '.rc1159-head{display:flex;gap:12px;align-items:flex-start;justify-content:space-between;flex-wrap:wrap}.rc1159-head h3{margin:0 0 4px}.rc1159-muted{color:#64748b;font-size:13px}'+
 '.rc1159-list{display:grid;gap:10px;margin-top:12px}.rc1159-row{padding:12px;border:1px solid #e2e8f0;border-radius:12px;background:#f8fafc;display:grid;gap:8px}.rc1159-row-top{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap}.rc1159-actions{display:flex;gap:8px;flex-wrap:wrap}.rc1159-badge{display:inline-flex;padding:3px 8px;border-radius:999px;background:#e2e8f0;font-size:12px}.rc1159-badge.ok{background:#dcfce7;color:#166534}.rc1159-badge.off{background:#fee2e2;color:#991b1b}'+
 '.rc1159-dialog{position:fixed;inset:0;z-index:120000;background:rgba(15,23,42,.65);display:grid;place-items:center;padding:16px}.rc1159-dialog-card{width:min(620px,100%);max-height:90vh;overflow:auto;background:#fff;border-radius:16px;padding:20px;box-shadow:0 24px 80px rgba(0,0,0,.28)}.rc1159-dialog label{display:grid;gap:6px;margin:10px 0}.rc1159-dialog input,.rc1159-dialog textarea{width:100%;box-sizing:border-box;padding:10px;border:1px solid #94a3b8;border-radius:8px}.rc1159-secret{padding:10px;border-radius:8px;background:#f1f5f9;word-break:break-all;font-family:ui-monospace,monospace}.rc1159-status[data-kind="bad"]{color:#b91c1c}.rc1159-status[data-kind="ok"]{color:#166534}'+
 '.rc1159-rights-grid{display:grid;grid-template-columns:minmax(180px,1fr) auto auto auto;gap:10px;align-items:center}.rc1159-rights-grid select{min-width:180px;padding:8px}@media(max-width:640px){.rc1159-rights-grid{grid-template-columns:1fr}.rc1159-actions .btn{flex:1 1 auto}}';
...
function portalRows(customer,portals,canManage){
 if(!portals.length)return'<div class="rc1159-muted">Für diesen Kunden ist kein Kundenportal hinterlegt.</div>';
 return'<div class="rc1159-list">'+portals.map(function(p){
  return'<div class="rc1159-row" data-portal-id="'+esc(p.id)+'"><div class="rc1159-row-top"><div><b>'+esc(p.name)+'</b><div class="rc1159-muted">'+esc(p.url)+'</div></div><span class="rc1159-badge '+(p.active!==false?'ok':'off')+'">'+(p.active!==false?'Aktiv':'Inaktiv')+'</span></div>'+
   '<div><span class="rc1159-badge">'+(p.hasUsername?'Benutzername hinterlegt':'Kein Benutzername')+'</span> <span class="rc1159-badge">'+(p.hasPassword?'Passwort hinterlegt':'Kein Passwort')+'</span></div>'+
   '<div class="rc1159-actions"><button type="button" class="btn" data-rc1159-open="'+esc(p.id)+'">Portal öffnen</button><button type="button" class="btn primary" data-rc1159-reveal="'+esc(p.id)+'">Zugangsdaten anzeigen</button>'+
   (canManage?'<button type="button" class="btn" data-rc1159-edit="'+esc(p.id)+'">Bearbeiten</button><button type="button" class="btn" data-rc1159-delete="'+esc(p.id)+'">Löschen</button>':'')+'</div></div>'
 }).join('')+'</div>'
}
...
function modal(html){clearRevealedSecrets();var box=d.createElement('div');box.id='rc1159RevealDialog';box.className='rc1159-dialog';box.innerHTML='<div class="rc1159-dialog-card">'+html+'</div>';box.addEventListener('click',function(e){if(e.target===box||e.target.closest('[data-rc1159-close]')){e.preventDefault();clearRevealedSecrets()}});d.body.appendChild(box);return box}
...
async function reveal(customer,p){
 var box=modal('<h3>Zugangsdaten anzeigen</h3><p class="rc1159-muted">Zur Sicherheit bitte dein persönliches ExportHUB-Passwort erneut eingeben.</p><label>ExportHUB-Passwort<input type="password" autocomplete="current-password" data-rc1159-reauth></label><div class="rc1159-status" data-rc1159-status></div><div class="rc1159-actions"><button class="btn" data-rc1159-close>Abbrechen</button><button class="btn primary" data-rc1159-confirm>Anzeigen</button></div>');
 var input=box.querySelector('[data-rc1159-reauth]'),out=box.querySelector('[data-rc1159-status]');
 input.focus();
 box.querySelector('[data-rc1159-confirm]').addEventListener('click',async function(){
  ...
  try{
   ...
   box.querySelector('.rc1159-dialog-card').innerHTML='<h3>'+esc(p.name)+'</h3><p class="rc1159-muted">Die Werte werden nach 60 Sekunden automatisch ausgeblendet.</p><b>Benutzername</b><div class="rc1159-secret" data-rc1159-secret="username">'+esc(data.username||'')+'</div><button class="btn" data-copy="username">Benutzername kopieren</button><br><br><b>Passwort</b><div class="rc1159-secret" data-rc1159-secret="password">'+esc(data.password||'')+'</div><button class="btn" data-copy="password">Passwort kopieren</button><div class="rc1159-actions" style="margin-top:16px"><button class="btn" data-rc1159-close>Schließen</button></div>';
   box.querySelectorAll('[data-copy]').forEach(function(btn){btn.addEventListener('click',function(){var n=box.querySelector('[data-rc1159-secret="'+this.getAttribute('data-copy')+'"]');copyText(n&&n.textContent)})});
...
function portalForm(customer,existing,onDone){
 ...
 '<label>Benutzername'+(existing?'<span class="rc1159-muted">leer lassen = unverändert</span>':'')+'<input data-f="username" autocomplete="off"></label>'+
 '<label>Passwort'+(existing?'<span class="rc1159-muted">leer lassen = unverändert</span>':'')+'<input data-f="password" type="password" autocomplete="new-password"></label>'+
 ...
 '<div class="rc1159-status" data-rc1159-status></div><div class="rc1159-actions"><button class="btn" data-rc1159-close>Abbrechen</button><button class="btn primary" data-save>Speichern</button></div>');
 box.querySelector('[data-save]').addEventListener('click',async function(){
  ...
  var out=box.querySelector('[data-rc1159-status]');this.disabled=true;status(out,'Speichern …');
  try{
...
function wirePortalBox(box,customer,portals,canManage,reload){
 box.querySelectorAll('[data-rc1159-open]').forEach(function(btn){btn.addEventListener('click',function(){openPortal(portals.find(function(p){return p.id===btn.getAttribute('data-rc1159-open')}))})});
 box.querySelectorAll('[data-rc1159-reveal]').forEach(function(btn){btn.addEventListener('click',function(){var p=portals.find(function(x){return x.id===btn.getAttribute('data-rc1159-reveal')});if(p)reveal(customer,p)})});
 box.querySelectorAll('[data-rc1159-edit]').forEach(function(btn){btn.addEventListener('click',function(){var p=portals.find(function(x){return x.id===btn.getAttribute('data-rc1159-edit')});if(p)portalForm(customer,p,reload)})});
 box.querySelectorAll('[data-rc1159-delete]').forEach(function(btn){btn.addEventListener('click',function(){var p=portals.find(function(x){return x.id===btn.getAttribute('data-rc1159-delete')});if(p)deletePortal(customer,p,reload)})});
}
async function renderCustomerFolder(customer,r){
 var root=d.getElementById('content');if(!root)return;
 var old=d.getElementById('rc1159CustomerFolderPortal');if(old)old.remove();
 var box=d.createElement('section');box.id='rc1159CustomerFolderPortal';box.className='rc1159-box';box.innerHTML='<div class="rc1159-head"><div><h3>Kundenportal</h3><div class="rc1159-muted">Zugangsdaten werden verschlüsselt außerhalb des Sendungs- und Team-State gespeichert.</div></div>'+(r.manage?'<button class="btn primary" data-add>Portal hinzufügen</button>':'')+'</div><div data-body class="rc1159-muted">Laden …</div>';root.appendChild(box);
 var seq=++loadSeq;
...
async function renderShipment(customer,r){
 ...
 var old=d.getElementById('rc1159ShipmentPortal');if(old)old.remove();
 var box=d.createElement('section');box.id='rc1159ShipmentPortal';box.className='rc1159-box';box.innerHTML='<div class="rc1159-head"><div><h3>Kundenportal</h3><div class="rc1159-muted">Portalzugang für den aktuell ausgewählten Kunden.</div></div></div><div data-body class="rc1159-muted">Laden …</div>';host.appendChild(box);
 var seq=++loadSeq;
...
async function renderRights(){
 ...
 var old=d.getElementById('rc1159PortalRights');if(old)old.remove();
 var box=d.createElement('section');box.id='rc1159PortalRights';box.className='rc1159-box';box.innerHTML='<div class="rc1159-head"><div><h3>Kundenportal-Rechte</h3><div class="rc1159-muted">Separates Funktionsrecht für sensible externe Zugangsdaten.</div></div></div><div data-body class="rc1159-muted">Benutzer laden …</div>';root.appendChild(box);
 var seq=++rightsSeq;
 try{
  var data=await authApi('admin-list',{});if(seq!==rightsSeq)return;var users=arr(data.users),body=box.querySelector('[data-body]');
  body.innerHTML='<div class="rc1159-rights-grid"><select data-user>'+users.map(function(u){return'<option value="'+esc(u.id)+'">'+esc(u.name||u.user)+'</option>'}).join('')+'</select><label><input type="checkbox" data-use> verwenden</label><label><input type="checkbox" data-manage> verwalten</label><button class="btn primary" data-save>Speichern</button></div><div class="rc1159-status" data-rc1159-status></div>';
  var select=body.querySelector('[data-user]'),use=body.querySelector('[data-use]'),manage=body.querySelector('[data-manage]'),save=body.querySelector('[data-save]'),out=body.querySelector('[data-rc1159-status]');
...
function removeBoxes(){['rc1159CustomerFolderPortal','rc1159ShipmentPortal','rc1159PortalRights'].forEach(function(id){var n=d.getElementById(id);if(n)n.remove()})}
...
w.ExportHUBCustomerPortal1159=Object.freeze({version:'RC1159',rights:rights,selectedCustomer:selectedCustomer,clearRevealedSecrets:clearRevealedSecrets,portalApi:portalApi,schedule:schedule});
})(window,document);
