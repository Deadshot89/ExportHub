...
function publicShipment(sh,session){
 const actual=dateTimeOf(sh),autoExpiresOn=avisExpiresOn(sh),docs=documentRecords(sh).map(d=>({id:d.id,name:d.name,mime:d.mime,size:d.size,category:d.category,downloadUrl:'/api/customer-avis?action=document&id='+encodeURIComponent(d.id)+'&session='+encodeURIComponent(session)})),podDocs=docs.filter(d=>d.category==='POD');
