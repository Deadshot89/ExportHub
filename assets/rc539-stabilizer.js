(function(){
'use strict';
if(window.__EXPORTHUB_RC539_STABILIZER__)return;
window.__EXPORTHUB_RC539_STABILIZER__=true;
...
function setVersionEverywhere(){
 try{document.title='ExportHUB RC539'}catch(_){ }
 document.documentElement.setAttribute('data-exporthub-version','RC539');
 document.querySelectorAll('[data-exporthub-version-label],[id*=version i],[class*=version i]').forEach(function(n){
  ...
  if(/(?:ExportHUB\s+Private\s+|Private\s+)?RC\d+/i.test(t))n.textContent=t.replace(/ExportHUB\s+Private\s+RC\d+/gi,'ExportHUB RC539').replace(/Private\s+RC\d+/gi,'RC539').replace(/RC\d+/gi,'RC539');
 });
}
function needsRepair(v,r){
 ...
 if(v==='abd')return !r.classList.contains('rc539-abd-view')||!r.querySelector('.rc539-abd-page');
...
function repair(reason){
 ...
 repairing=true;
 try{
  var core=window.ExportHUBRC539||window.ExportHUBRC538||window.ExportHUBRC532;
  ...
  window.__EXPORTHUB_RC538_UI_STATUS__={ok:true,view:v,reason:reason||'repair',version:'RC539',checkedAt:new Date().toISOString(),areas:{shipmentOrder:'Kunde/Grunddaten > Collis > Stauplan > Mail > Reports > Gespeicherte Sendungen',abd:'eigener Bereich'}};
 }catch(e){console.error('RC539 Oberflächenreparatur',e);window.__EXPORTHUB_RC538_UI_STATUS__={ok:false,error:String(e&&e.message||e),view:v}}
