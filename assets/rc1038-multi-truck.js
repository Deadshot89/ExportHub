(function(root){
  ...
  function planSubShipments(options){
    ...
    if(!rows.length){
      return {requiredTruckCount:1,subShipments:[],changed:previous.length>0,locked:false};
    }

    const units=explodeRows(rows);
    if(fits(input.fitRows,units)){
      return {requiredTruckCount:1,subShipments:[],changed:previous.length>0,locked:false};
    }
    ...
    units.forEach(function(unit){
      ...
      if(!placed){
        if(!fits(input.fitRows,[unit])){
          throw error('PHYSICAL_UNIT_EXCEEDS_CAPACITY','Eine physische Einheit passt nicht in das ausgewählte Fahrzeug.');
        }
        trucks.push([unit]);
      }
    });
    ...
    return {
      requiredTruckCount:total,
      subShipments:subShipments,
      changed:JSON.stringify(previous)!==JSON.stringify(subShipments),
      locked:false
    };
  }
