...
test('RC1160 Security Gate: keine fest verdrahteten Portal-Secrets',()=>{
  assert.doesNotMatch(joined,/SECRET-(?:USER|PASS)-RC1160/);
  assert.doesNotMatch(joined,/EXPORTHUB_CUSTOMER_PORTAL_KEY\s*[:=]\s*['"][A-Za-z0-9+/=_-]{16,}['"]/i);
  assert.doesNotMatch(fs.readFileSync('assets/rc1160-customer-portal-credentials.js','utf8'),/password\s*:\s*['"][^'"]{3,}['"]/i);
});
test('RC1160 Security Gate: Team-State erhält keine Portal-Credentials',()=>{
...
test('RC1160 Security Gate: Verschlüsselung ist AES-256-GCM und Schlüssel bleibt serverseitig',()=>{
  ...
  assert.doesNotMatch(fs.readFileSync('assets/rc1160-customer-portal-credentials.js','utf8'),/EXPORTHUB_CUSTOMER_PORTAL_KEY/);
});
test('RC1160 Security Gate: Beispielkonfiguration enthält nur Platzhalter',()=>{
