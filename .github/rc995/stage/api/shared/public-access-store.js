...
async function issue(req,kind,meta={},ttlMs,payload){
  ...
  const token=crypto.randomBytes(32).toString('base64url'),tokenHash=hashToken(token,env,kind),createdAt=now(),ttl=Math.max(60*1000,Number(ttlMs)|| (kind==='pickup'?DEFAULT_PICKUP_TTL_MS:DEFAULT_AVIS_TTL_MS)),expiresAt=new Date(Date.now()+ttl).toISOString();
